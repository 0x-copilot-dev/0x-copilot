...
def make_event_emitter(
    *,
    ...
    runtime events. The ``store`` param is retained for signature compatibility;
    the v1 ``message`` surface envelope that was attached here was retired in E3
    (D4) — draft surfaces now render from the D1-wave ``write.staged`` /
    ``revision.added`` ledger events, not a ``DRAFT_UPDATED`` payload decoration.
    """
