...
PUBLISH_ARTIFACT_TOOL_DESCRIPTION = (
    ...
    "Fields:\n"
    "- `kind` (required): `code`, `document`, `dataset`, or `file`. It has to "
    "match the media type, because it picks the renderer: CSV and TSV are "
    "`dataset`, Markdown is `document`, source files are `code`. `file` is only "
    "for content none of those can show — an archive, an image, a binary. "
    "Publishing a CSV as `file` is refused rather than shown without its "
    "table.\n"
