...
class FileArtifactPublicationCoordinator:
    ...
    def pending_candidates_locked(
        ...
        """Return the durable pending-publication manifest in O(pending) time.

        New blob publication records a manifest entry and fsyncs it *before*
        the atomic rename.  That entry is the only source of ordinary
        orphan-recovery work: it survives a crash but is retired once a
        metadata reference has been observed.  In particular, this method
        must never enumerate every healthy active blob.  A physical inventory
        reconciliation is an explicit maintenance operation, not a retention
        sweep side effect.
        """
