"""File-backed ``ShareStorePort`` — durable conversation shares + recipients.

Two ledgers: ``state/shares.jsonl`` (fold-by-``share_id``) and
``state/share_recipients.jsonl`` (a whole-set snapshot per share, fold-by-
``share_id`` — recipient lists are replaced atomically, never appended to).
"""
...
class FileShareStore:
    """Durable, single-writer share store backed by two JSONL ledgers."""

    _SHARES = "shares"
    _RECIPIENTS = "share_recipients"

    def __init__(self, layout: FileStoreLayout) -> None:
        self._lock = RLock()
        self._shares_ledger = StateLedger(layout.state_path(self._SHARES))
        self._recipients_ledger = StateLedger(layout.state_path(self._RECIPIENTS))
        self.shares: dict[str, ShareRecord] = {}
        self.recipients: dict[str, dict[str, ShareRecipientRecord]] = {}
        self._load()

    def _load(self) -> None:
        for record_json in self._shares_ledger.load_puts():
            record = ShareRecord.model_validate(record_json)
            self.shares[record.share_id] = record
        for snapshot in self._recipients_ledger.load_puts():
            share_id = snapshot.get("share_id")
            rows = snapshot.get("recipients", [])
            if not isinstance(share_id, str) or not isinstance(rows, list):
                continue
            self.recipients[share_id] = {
                recipient.user_id: recipient
                for recipient in (
                    ShareRecipientRecord.model_validate(row) for row in rows
                )
            }

    def _persist_recipients(self, share_id: str) -> None:
        current = self.recipients.get(share_id, {})
        self._recipients_ledger.append_put(
            {
                "share_id": share_id,
                "recipients": [r.model_dump(mode="json") for r in current.values()],
            }
        )

    async def insert_share(
        self, *, share: ShareRecord, recipients: Sequence[ShareRecipientRecord]
    ) -> ShareRecord:
        with self._lock:
            ...
            self._shares_ledger.append_put(share.model_dump(mode="json"))
            self._persist_recipients(share.share_id)
            return share
    ...
    async def replace_recipients(
        ...
        *,
        ...
        with self._lock:
            ...
            self._persist_recipients(share_id)
    ...
    async def update_share(
        ...
        *,
        ...
        with self._lock:
            ...
            self._shares_ledger.append_put(updated.model_dump(mode="json"))
            return updated
