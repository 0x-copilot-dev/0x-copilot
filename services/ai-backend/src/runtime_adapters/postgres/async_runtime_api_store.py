...
Mirrors :mod:`runtime_adapters.postgres.runtime_api_store` (sync, psycopg 3 +
``ConnectionPool``) using ``psycopg.AsyncConnection`` and
``psycopg_pool.AsyncConnectionPool``. Same SQL, same tables, same row mappers.
The deltas from the sync version are:

- ``async with self._pool.connection() as conn:`` + ``async with
  conn.transaction():`` for transactional cancellation safety.
- ``append_event`` keeps the existing ``SELECT … FROM agent_runs … FOR
  UPDATE`` pattern (H1) so concurrent appends per run serialize on the
  ``agent_runs`` row lock; the UNIQUE on ``runtime_events(run_id,
  sequence_no)`` is the load-bearing safety net.
- ``create_approval_request`` is rewritten as ``INSERT … ON CONFLICT (id) DO
  NOTHING`` followed by a ``SELECT`` of the existing row (H2). The previous
  check-then-insert pattern was racy under concurrent producers.
- ``set_run_latest_sequence`` is monotonic: the UPDATE has ``WHERE id = $1
  AND latest_sequence_no < $2``. Out-of-order writes from concurrent appenders
  no longer rewind the cursor (H3).
"""
...
from runtime_adapters.postgres.runtime_api_store import _Columns
...
class AsyncPostgresRuntimeApiStore:
    ...
    async def __aenter__(self) -> AsyncPostgresRuntimeApiStore:
        await self.open()
        return self
