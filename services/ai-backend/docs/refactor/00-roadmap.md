...
| #   | PR                                                  | Audit ref                                                                                                                                                                                                                                                                                                | PRD                                  | Risk   | Behaviors preserved                                                                                                                                                                              |
| --- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| P14 | Citation infrastructure consolidation (8 files → 3) | [§2.2](../architecture/refactor-audit.md#22-8-files-of-citation-infrastructure)                                                                                                                                                                                                                          | TBD: `11-citations-consolidation.md` | Medium | Conversation-scoped ordinal namespace across turns AND subagents; idempotency `(tool_call_id)` and `(run_id, connector, doc_id)`; sealed snapshot at FINAL_RESPONSE; rebuild from `list_for_run` |
| P15 | Worker streaming pipeline cleanup                   | [§5.1](../architecture/refactor-audit.md#51-worker-side-toolcallledger-duplicates-persistence-side-toolinvocationstoreport), [§5.2](../architecture/refactor-audit.md#52-approvalrecognisers-in-the-worker), [§5.3](../architecture/refactor-audit.md#53-streaming-pipeline--10-files-inside-the-worker) | TBD: `12-worker-stream-cleanup.md`   | Medium | Typed approval events emitted by tool / middleware (not stream pattern matching); `action_interrupt_events` set; `ToolInvocationRecord` as single source of truth                                |
| P16 | Eliminate `SELECT FOR UPDATE` via per-run sequence  | [§4.3](../architecture/refactor-audit.md#43-per-event-db-amplification) (point 2)                                                                                                                                                                                                                        | TBD: `13-per-run-sequence.md`        | Medium | Strict per-run monotonic `sequence_no`; `UNIQUE(run_id, sequence_no)` constraint                                                                                                                 |

**Phase exit criterion:** citations are 3 files; worker streaming has no duplicate ledgers / pattern recognizers; per-run sequence allocation runs without row locks at p99 ≤ Phase 1 baseline.
...
- **P4 → P16:** the `INSERT … RETURNING` pattern from P4 is the foundation for the per-run sequence allocator in P16.
...
- **P14 + P15 → P22:** citation and streaming-pipeline consolidations remove dependents from the coordinator before it splits.
