...
| **[01c — Single UsageRecorder boundary](01c-usage-recorder.md)** ✅ shipped 2026-05-11                   | Defined `UsageRecorder` Protocol + `UsageRecordingResult` typed return + three impls (`PostgresUsageRecorder` prod, `InMemoryUsageRecorder` tests, `NullUsageRecorder` replay). Collapsed `_record_run_usage` + `_record_per_call_usage` in `handlers/run.py` into one coordinator that delegates to `self.usage_recorder`. Both writes share one `pricing_at=completed_at` so a minute-boundary clock crossing mid-run can't stamp two pricing versions. Added `SummarizationUsageRecorder` scaffold (D4 boundary) — production summarization is dead code today; scaffold means future enable is a one-line wire-up. Budget charger reads `result.cost_micro_usd` from the recorder's typed return — no mutable-local passing. **Deleted** `_record_per_call_usage` + duplicate pricing-lookup blocks + orphaned `CostCalculator` import in handler. | Medium      | 01b        |
| **[01d — Rollup expansion](01d-usage-rollup-expansion.md)** ✅ shipped 2026-05-11 (FE deferred)          | Added `model_name` to `UsageDailyConnectorRow` (PK extension via migration 0029). New `UsageDailySubagentRow` + `UsageDailyPurposeRow` (each carries all 7 token kinds). New `/v1/usage/org/subagents` + `/v1/usage/org/purpose` endpoints (admin-or-auditor scope, cold-start fallback pattern). `UsageQueryService.rollup_subagent_rows` + `rollup_purpose_rows` builders with `_SubagentRollupBucket` + `_PurposeRollupBucket`. Rollup loop refresh blocks added. api-types contracts (`UsageSubagentRow`, `UsagePurposeRow`, `UsageOrgSubagentsResponse`, `UsageOrgPurposeResponse`). Facade proxies wired. **FE sections (two new tables in `UsageWorkspaceView`) deferred to a separate frontend PR — data is queryable; presentation is pure follow-up.**                                                                                       | Medium      | 01c        |
