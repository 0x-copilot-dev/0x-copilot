...
| ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------- |
| **[01a — Normalized token shape](01a-usage-normalized-token-shape.md)**            | Add `NormalizedTokenUsage` value object. Add `TokenUsageExtractor` Protocol with per-provider implementations (OpenAI, Anthropic, Google). Add columns to `RuntimeModelCallUsageRecord` for `reasoning_tokens`, `cache_creation_input_tokens`, `audio_input_tokens`, `audio_output_tokens`, all defaulting to 0. Alembic migration. **Delete** the in-line extraction in `streaming_executor.py:154-186` and the `_MessageIdExtractor` provider-coupling — replace with the registry. No attribution changes yet.                                                | Low         | —          |
| **[01b — Carry attribution; delete heuristics](01b-usage-attribution-context.md)** | Define `UsageAttributionContext` + `Purpose` enum (`MAIN / TOOL_PLANNING / TOOL_INTERPRETATION / SUBAGENT_WORK / CONTEXT_COMPRESSION` with deterministic `Purpose.derive`). Build the context at every LLM call site. Pull `task_id` + `subagent_slug` from the LangGraph chunk namespace (kills D3 parallel-subagent bug). Add columns to record: `subagent_slug`, `originating_tool_call_id`, `originating_tool_name`, `purpose`. **Delete `UsageAttributionResolver`**. **Delete the `active_subagent_tasks` set arbitration.** Direct cutover — no fallback. | Medium-High | 01a        |
