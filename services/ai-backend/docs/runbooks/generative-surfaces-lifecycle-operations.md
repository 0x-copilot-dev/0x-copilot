...
It does **not** claim that a durable lifecycle planner runner, repair runner,
physical cleanup executor, scheduled audit sampler, dashboard, or alert
delivery integration has shipped. This repository only exports OpenTelemetry
...
| `surfaces_lifecycle_reconcile_backlog_snapshot_items`   | Observable gauge | `state`                                    | Repair planner                          | Latest process-local count of D12 **effect-reconciliation** candidates/withheld rows. It resets after restart and is meaningful only once a scheduler invokes the planner. |
...
| `LifecycleReconcileBacklog`          | `max_over_time(surfaces_lifecycle_reconcile_backlog_snapshot_items{state="candidate"}[30m]) > 0` for 30m   | Warning to Effect Reconciliation owner        | This is informational until the D12 scheduled runner lands. Once a runner is enabled, inspect only redacted candidate evidence and use the existing reconcile path; never apply or resend an uncertain effect.            |
...
3. Use the existing reconciler once the scheduled runner and authorization
   layer are deployed. Never turn a metric or candidate into a direct external
   effect.
4. If the process restarted, wait for the next trusted planning invocation;
   the current gauge is intentionally process-local.
...
- durable snapshot collection and scheduled D10/D12 planner/repair jobs;
