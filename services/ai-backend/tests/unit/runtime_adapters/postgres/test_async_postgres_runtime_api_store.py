"""End-to-end tests for AsyncPostgresRuntimeApiStore against a real Postgres.
...
from runtime_adapters.postgres import AsyncPostgresRuntimeApiStore
...
pytestmark = [
    pytest.mark.skipif(
        ...
        reason="TEST_DATABASE_URL is required for AsyncPostgresRuntimeApiStore tests.",
    ),
]
...
@pytest.fixture
async def store() -> AsyncIterator[AsyncPostgresRuntimeApiStore]:
    """Open a fresh async store, run migration, hand it to the test."""

    s = AsyncPostgresRuntimeApiStore(
...
async def _seed_run(
    store: AsyncPostgresRuntimeApiStore, suffix: str | None = None
...
class TestAsyncAdapterParity:
    ...
    async def test_list_events_after(self, store: AsyncPostgresRuntimeApiStore) -> None:
        org_id, _user_id, run_id, conv_id = await _seed_run(store)
        for i in range(5):
