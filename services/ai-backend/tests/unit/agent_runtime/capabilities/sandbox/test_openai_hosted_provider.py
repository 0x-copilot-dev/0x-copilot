...
class _Containers:
    ...
    async def list(self, *, name: str, limit: int, after: str | None = None) -> object:
        del limit, after
        data = [
            SimpleNamespace(**record)
...
class TestOpenAIHostedContainerProvider:
    async def test_create_uses_disabled_network_and_captures_response_evidence(
        ...
        first = await provider.create(make_request())
        second = await provider.create(make_request())
    ...
    async def test_create_compiles_only_the_explicit_allowlist(self) -> None:
        client = _Client()
        config = _config(
            network_policy={
                "mode": "allowlist",
        ...
        provider = OpenAIHostedContainerProvider(config=config, client=client)
    ...
    async def test_transfers_only_sealed_bytes_and_collects_declared_output(
        ...
        assert isinstance(backend, OpenAIHostedContainerBackend)
        ...
        evidence = backend.execution_evidence
    ...
    async def test_async_timeout_cancels_the_concrete_response(self) -> None:
    ...
    async def test_status_list_owned_and_reap_are_container_scoped(self) -> None:
        client = _Client()
        provider = OpenAIHostedContainerProvider(config=_config(), client=client)
        handle = await provider.create(make_request(owner_tag="owner-a"))
        await provider.create(
            make_request(idempotency_key="other", owner_tag="owner-b")
        )

        status = await provider.status(handle.session.provider_session_ref)
        owned = await provider.list_owned_sessions("owner-a")
        await provider.terminate(handle.session.provider_session_ref)
        await provider.terminate(handle.session.provider_session_ref)
    ...
    async def test_output_is_bounded_by_the_existing_policy_wrapper(self) -> None:
        client = _Client(output_text="x" * 128)
        provider = OpenAIHostedContainerProvider(config=_config(), client=client)
        handle = await provider.create(make_request())
        policy = PolicyEnforcedSandboxBackend(
            delegate=handle.backend,
