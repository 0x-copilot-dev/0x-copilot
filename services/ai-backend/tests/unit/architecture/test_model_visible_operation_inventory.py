...
def test_every_assembled_model_tool_has_one_catalog_descriptor(
    ...
    """Cover every composed model tool against the reviewed catalog.

    Asserting set *equality* against the catalog's model-visible entries means a
    row nothing composes is as much a failure as a tool nothing catalogs, so
    neither a stranded catalog row nor a flag-gated tool can hide.
    """
