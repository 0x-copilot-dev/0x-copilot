-- Initial agent runtime persistence schema.
--
-- Verbatim port of POSTGRES_AGENT_RUNTIME_MIGRATION_SQL that historically
-- lived inline in services/ai-backend/src/agent_runtime/persistence/schema/postgres.py.
-- Uses CREATE TABLE IF NOT EXISTS so it stays idempotent on databases that
-- were previously initialised by the legacy migrate() path.
