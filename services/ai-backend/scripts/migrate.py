"""Operator CLI for ai-backend service migrations.
...
Examples:
    ...
    .venv/bin/python scripts/migrate.py rollback --to 0001_initial_runtime_persistence

    # Show applied/pending status
    .venv/bin/python scripts/migrate.py status

The DB URL is read from ``RUNTIME_DATABASE_URL``; pass ``--db-url`` to override.
"""
...
_LOGGER = logging.getLogger("ai-backend.migrate")


def _resolve_database_url(args: argparse.Namespace) -> str:
    if args.db_url:
        return args.db_url
    url = os.environ.get("RUNTIME_DATABASE_URL", "").strip()
    if not url:
        sys.stderr.write(
            "ERROR: set RUNTIME_DATABASE_URL or pass --db-url=postgresql://...\n"
        )
        raise SystemExit(2)
    return url
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="ai-backend-migrate")
    parser.add_argument("--db-url", default=None, help="Override RUNTIME_DATABASE_URL")
