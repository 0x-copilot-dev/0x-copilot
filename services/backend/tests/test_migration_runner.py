"""Tests for the backend yoyo-migrations runner.

The Postgres-only DDL in production migrations cannot run against SQLite
(JSONB, GENERATED, partial indexes, etc.), so we exercise the runner against
fixture migrations checked in under ``tests/fixtures/migrations/``. Production
migrations are exercised end-to-end by the integration suite that boots a real
Postgres.
"""
...
class TestMigrationRunner:
    def test_apply_then_rollback_roundtrip_against_sqlite(self, tmp_path: Path) -> None:
        ...
        # yoyo returns migrations in apply (forward) order from to_apply().
        assert applied == ["0001_create_widgets", "0002_add_widget_color"]

        applied_after, pending_after = _SqliteRunner.status(url)
        # to_rollback() returns reverse order so callers can roll back safely;
        # we don't pin the order here, just the set.
    ...
    def test_auto_apply_false_when_explicitly_disabled(self) -> None:
        assert (
            MigrationRunner.auto_apply_enabled(
                env={"BACKEND_MIGRATIONS_AUTO_APPLY": "false"}
            )
            is False
        )
...
class TestProductionManifestMatchesDirectory:
    ...
    def test_missing_manifest_raises_typed_error(
        ...
        from backend_app.db import migrate as migrate_module
