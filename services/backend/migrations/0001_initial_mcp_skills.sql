-- Initial backend schema: MCP registry + skills.
--
-- This migration is the verbatim port of the historic SQL strings that lived
-- inline in services/backend/src/backend_app/migrations.py. It uses CREATE
-- TABLE IF NOT EXISTS so it stays idempotent on databases that were
-- previously initialised by the legacy migrate() path.
