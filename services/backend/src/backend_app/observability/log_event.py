"""Pydantic-validated structured log records for the backend service.

The shape is fixed: free-form ``message`` text is intentionally absent. Callers
emit a short ``event`` label (e.g. ``mcp.server.created``) plus typed fields.
...
class LogEvent(BaseModel):
    """One structured log record; serialized as JSON by the formatter."""
...
class MetadataRedactor:
    """Drop sensitive keys and reject non-scalar values.

    Why: developers occasionally pass a dict directly from a request body or
    SDK response. The denylist refuses to log anything keyed by an obvious
    sensitive name; the type filter refuses lists/dicts/objects so a stray
    payload can never serialize through this path.
    """
