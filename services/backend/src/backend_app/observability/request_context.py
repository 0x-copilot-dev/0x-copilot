...
The ASGI middleware reads (or generates) ``x-request-id`` on ingress and
publishes a ``RequestContext`` for the lifetime of the request so any
downstream ``get_logger().info(...)`` call inherits the correlation IDs without
threading them through call sites. One access log line is emitted per request
with method, route template, status, and duration -- no path, no query, no
body, no headers.
"""
...
_REQUEST_CONTEXT: ContextVar[RequestContext | None] = ContextVar(
    "backend_app_request_context", default=None
)


def current_context() -> RequestContext | None:
    """Return the current request context if one is bound, else ``None``."""
...
class RequestContextMiddleware:
    ...
    Pure ASGI (not Starlette ``BaseHTTPMiddleware``) because the latter buffers
    streaming responses, which would break SSE on the facade. The shape mirrors
    Starlette's contract so it composes with the FastAPI app cleanly.
    """
