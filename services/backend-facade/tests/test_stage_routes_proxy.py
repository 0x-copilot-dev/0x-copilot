...
def test_facade_proxies_generic_mcp_effect_decision_verbatim(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured = _install_capturing_forwarder(
        monkeypatch,
        expected_path=f"/v1/agent/effect-stages/{_STAGE_ID}/decision",
        expected_method="POST",
    )
    client = TestClient(create_app(FacadeSettings()))
    body = {
    ...
    response = client.post(
        f"/v1/agent/effect-stages/{_STAGE_ID}/decision?run_id={_RUN_ID}",
        headers=_headers(),
        json=body,
    )
