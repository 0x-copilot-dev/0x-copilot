"""Tests for the public ``/v1/inbox`` facade proxy (Phase 4 P4-A1).

Mirrors ``test_todos_proxy.py`` setup. Asserts:

* Unauthenticated request rejected (401).
* Authenticated request proxies to backend with the verified identity.
* Multi-value ``filter[state]`` query params survive the proxy
...
class TestInboxProxy:
    def test_get_list_proxies_to_backend(self, monkeypatch) -> None:
        ...
        class _FakeAsyncClient:
            ...
            async def get(self, url, *, params, headers, timeout=None):
                ...
                return httpx.Response(200, json=_INBOX_LIST_BODY)
        ...
        # Multi-value filter[state] survives the proxy.
        resp = client.get(
            "/v1/inbox?filter[state]=unread&filter[state]=snoozed",
            headers=_bearer_headers(monkeypatch),
        )
        assert resp.status_code == 200
        assert resp.json() == _INBOX_LIST_BODY

        get_call = next(c for c in captured if c["method"] == "GET")
        assert get_call["url"].endswith("/v1/inbox")
        pairs = get_call["params"]
        assert ("org_id", "org_acme") in pairs
        assert ("user_id", "usr_sarah") in pairs
        # Multi-value preserved.
        assert pairs.count(("filter[state]", "unread")) == 1
        assert pairs.count(("filter[state]", "snoozed")) == 1
    ...
    def test_unread_count_proxies(self, monkeypatch) -> None:
        captured: list[str] = []
        ...
        class _FakeAsyncClient:
            ...
            async def get(self, url, *, params, headers, timeout=None):
                ...
                return httpx.Response(
                    200,
                    json={"unread_count": 3, "as_of": "2026-05-18T00:00:00+00:00"},
                )
        ...
        assert any(url.endswith("/v1/inbox/unread_count") for url in captured)
    ...
    def test_upstream_404_propagates(self, monkeypatch) -> None:
        class _FakeAsyncClient:
            ...
            async def patch(
                ...
                return httpx.Response(404, json={"detail": "inbox_item_not_found"})
        ...
        class _FakeAsyncClient:
            ...
            async def patch(
                ...
                return httpx.Response(
                    200,
                    json={
                        **_INBOX_LIST_BODY["items"][0],
                        "state": "read",
                        "read_at": "2026-05-18T01:00:00+00:00",
                    },
                )
