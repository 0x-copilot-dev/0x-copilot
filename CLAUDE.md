...
- `agent_runtime/` — pure domain, 21 packages. Runtime core: `execution/` (graph, deep agent builder, runtime contracts), `capabilities/` (tools, skills, MCP loaders + middleware + permissions), `context/memory`, `delegation/subagents`, `persistence/` (records + ports), `observability/`, `api/` (presentation/service layer for the runtime API), `prompts/`, `control_plane/`, `hyperparameters/`, `deployment/`. Generative Surfaces: `surfaces_v2/` (the Work Ledger), `effects/` (pure staging domain), `artifacts/`, `presentation/`. Known-misplaced, target home `backend`: `harness_quality/`, `pricing/`, `budgets/`, `release/`, `retention/` — see [BOUNDARY-AUDIT.md](docs/audit/ai-backend-smells/BOUNDARY-AUDIT.md).
