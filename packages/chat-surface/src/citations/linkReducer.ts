// Model-declared citation link reducer.
//
// Citation links are pointers (assistant_message_id + prose_offset →
// tool_invocation by conversation_ordinal). Substrate-agnostic: pure
// reduction over RuntimeEventEnvelope events, no console / no DOM / no
// app-specific dependencies. Diagnostic emissions go through an injected
// `onDebug` callback so the substrate (web's citationDebug, desktop's
// extension-side telemetry) decides where they land.
//
...
/** Diagnostic sink. Web wires `citationDebug`; the desktop substrate can
 *  bind it to its own telemetry channel. */
export type CitationLinkDebug = (message: string, detail?: unknown) => void;

const NOOP_DEBUG: CitationLinkDebug = () => {};
 ...
 *
 *  `source_tool_call_id` may arrive as `undefined` or `null` for two
 *  reasons:
 *  - events persisted before the projector defaulted the field to `""`
 *    (replay path),
 *  - the resolver fired for a hallucinated ordinal the allocator never
 *    bound to a tool call.
 *  Downstream consumers (chip resolution, tool-source projection) rely
 *  on the field being a string, so coerce here once. */
...
export function buildCitationLinkRegistry(
  ...
  for (const event of events) {
    ...
    registry = applyCitationLinkEvent(registry, event, onDebug);
  }
  onDebug(
