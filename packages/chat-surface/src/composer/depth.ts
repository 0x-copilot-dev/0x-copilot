...
// Hoisted from apps/frontend/src/features/chat/depth.ts (PR-1.2).
//
// FR-1.7 flag — this `ThinkingDepth` model coexists with the base
// `chat-surface` `Depth` / `listDepthDescriptors` (composer/ModelPicker).
// They are NOT the same enum: `ThinkingDepth` is the advanced-composer /
// topbar three-segment control whose selection flows to the run-start
// payload as the `reasoning_depth` wire field, while `Depth` is the base
// Composer's own descriptor list. Phase 1 keeps both and re-exports this
// one so `apps/frontend` stops re-declaring it; the reconciliation of the
// two depth models is deferred to Phase 3E (do not silently fork them).
