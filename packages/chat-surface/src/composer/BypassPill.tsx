...
//
// Two lanes built this component independently and this file is the merge of
// them. PRD-FS-10 shipped SELECTION ONLY — a `manual | bypass` pill mounted
// inside `AssistantComposer` off data props, with nothing downstream reading
// the value. PRD-FS-11 shipped the BEHAVIOUR, and with it the two things the
// selection-only version could not have: a SCOPE (message vs run — see
// `filesystemBypass.ts` on stickiness) and a host-owned mount, because the
// master switch is server-held and this package may not fetch it.
//
// The FS-11 shape is the one kept: it is a superset, and it is the one the
// desktop host, the run-create seam and the runtime actually speak. What
// survived from FS-10 is the rule its header stated best, and it is the reason
// the disabled state matters more than the enabled one — until the Settings
// master is on, this must not offer Bypass at all. An option that is offered
// and then ignored is worse than an absent one, and "the user said bypass" is
// exactly the sort of claim something downstream later reads as authorization.
