...
| An **indeterminate activity spinner** (in-flight work)           | `.ui-spinner` (+ `--ui-spinner-size` / `--ui-spinner-stroke`)                     | `<Spinner size stroke>`     | accent ring on a 35% accent track · 0.8s linear · decorative (`aria-hidden`)                             |
