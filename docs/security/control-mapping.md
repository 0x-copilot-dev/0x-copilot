...
| Supply-chain gates                     | `.github/workflows/ci-repo.yml` (gitleaks, pre-commit), `ci-backend.yml` / `ci-backend-facade.yml` / `ci-ai-backend.yml` / `ci-frontend.yml` (pip-audit / npm audit, SBOM), `release-images.yml` (Trivy CRITICAL gate, cosign signing, build provenance attestation), `CODEOWNERS`, Docker non-root users | CC8.1        | A.8.8, A.8.9   | ID.RA-01, PR.PS-06 | V14        |
