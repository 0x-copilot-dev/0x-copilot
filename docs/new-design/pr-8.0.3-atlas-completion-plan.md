...
| **B5 — audit_events_view**                      | ✅ Endpoint already shipped (PR 7.1) | `GET /v1/audit` works end-to-end. The new SQL view in `0025_audit_events_view.sql` is an **optional read-model** the existing endpoint can adopt later for cleaner SQL.                                                    |
