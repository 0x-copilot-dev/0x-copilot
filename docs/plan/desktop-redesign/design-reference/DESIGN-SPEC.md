...
- **Provider keys (BYOK)** — Connected list (logo, name + model chip, masked key, Rotate / Remove); Add-a-provider list (empty providers → "Add key"); "Another provider — **Any OpenAI-compatible endpoint works too.**" Note: "Keys stored in your **macOS Keychain**, encrypted at rest, never sent to a 0xCopilot server." Providers: Anthropic, OpenAI, OpenRouter, Google AI, Groq, xAI.
