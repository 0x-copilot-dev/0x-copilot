...
| F-002 | The sandbox must be desktop and filesystem first; selected bytes must be sealed and verified before any provider receives them.                         | [D3 contract](prds/PRD-D3-sandbox-adapter.md), [desktop file-store migration](../../operations/desktop-file-store-migration.md), current D3 draft audit (2026-07-26): `snapshot_file_store.py` validates size/digest only after yielding chunks upstream; its Postgres overlay adapter intentionally returns no retained manifest history.                                    | Retain the file-backed lifecycle/session/usage/cleanup namespace and C1/C3-only local mutation. Before an upload, materialize each selected input into a bounded worker-owned sealed object, verify its exact digest/size, then stream only that object. Non-file/Postgres history remains unavailable; do not add a fallback.                                                              | **Release blocker — correction in progress.**           |
...
| F-005 | D3 has a typed gateway/tool seam but no verified production worker composition, so the model-visible sandbox capability is correctly unavailable.       | Current D3 draft audit (2026-07-26): `runtime_worker/capability_tool_wiring.py` exposes an injected `SandboxToolFactoryPort`, but `runtime_worker/handlers/run.py` does not bind one; the tool is therefore absent.                                                                                                                                                           | Compose exactly one file-native `SandboxWorkerBundle` from verified C1/A2 snapshot authority, file-only lifecycle stores, and an attested provider gateway. The model-facing tool may call only the canonical coordinator path; direct `session_scope`/`aexecute`, PostgreSQL-history fallback, and automatic patch import remain prohibited. Add composition and no-pre-upload-byte tests. | **Release blocker — correction in progress.**           |
