...
| F-006 | Draft-send has two competing sources of truth: it stages legacy mutable draft bytes rather than the selected immutable Artifact revision.               | Current-main audit, 2026-07-26: `agent_runtime/api/draft_service.py::_stage_send_v2` passes `DraftRecord` into legacy `surfaces_v2/staging.py::WriteStager`.                                              | Rework B1 draft-send so the proposal binds an Artifact revision/content digest and the effect coordinator receives only that immutable revision. Do not patch this by copying bytes again at approval time.                                 | **Release blocker.**             |
...
| A4/A5 effects         | Core staging, coordinator ordering, claims, reconciliation, and sole `effect.applied` producer are proven.                                                        | Resolve F-009 and establish whole-suite zero-effect proof while staging.                                   |
...
| B2/B3 UI              | Safe renderer/editor and lifecycle unit coverage is proven.                                                                                                       | Resolve F-007 and F-008 before release; add a Focus/Studio projector differential assertion.               |
