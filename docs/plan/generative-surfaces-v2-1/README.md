...
| F-002 | The sandbox must be desktop and filesystem first; selected bytes must be sealed and verified before any provider receives them.                         | [D3 contract](prds/PRD-D3-sandbox-adapter.md), [desktop file-store migration](../../operations/desktop-file-store-migration.md), independent D3 foundation review (2026-07-26): sealing prevents mismatched bytes, nonzero retained-history pointer gaps fail closed, and queued context is checked against persisted run identity; however, a version-zero empty plan can still execute and cleanup-duty persistence failure after provider creation can lose the only provider reference. | Retain the file-backed lifecycle/session/usage/cleanup namespace and C1/C3-only local mutation. Reject any snapshot with no sealed selected input before upload/execution; distinguish genuinely new workspace state from pointer/history corruption; make provider cleanup recovery durable before the provider can be lost; and bind org/user/run/version exclusively from the verified persisted run before any snapshot/provider work. Non-file/Postgres history remains unavailable; do not add a fallback.                                                                                | **Release blocker — correction in progress.**           |
...
| F-005 | D3 has a typed gateway/tool seam but no verified production worker composition, so the model-visible sandbox capability must remain unavailable.        | Independent D3 provider review (2026-07-26): no repository provider attests the required controls; the candidate lacks full C1 base-plus-overlay export, durable A2 deliverable publication, explicit C1 patch import, and an enabled worker composition. Final foundation review additionally found a composition path that can report the tool available despite those missing prerequisites, while its runner hard-codes no deliverables and its coordinator has no importer.            | Compose exactly one file-native `SandboxWorkerBundle` from verified C1/A2 authority, concrete trusted file-store identities, a durable restart-safe cleanup reaper, a real patch collector, and an externally verifiable provider gateway. Make availability depend on every prerequisite. Add bounded base-plus-overlay snapshot export, result/deliverable publication, and explicit user-triggered C1 patch import; then reuse C3/C2’s existing approval/commit path. The model-facing tool may call only the coordinator; no direct executor, Postgres fallback, or automatic patch import. | **Release blocker — correction in progress.**           |
| F-006 | Draft-send has two competing sources of truth: it stages legacy mutable draft bytes rather than the selected immutable Artifact revision.               | Current `main` still stages `DraftRecord` through legacy `WriteStager`. Draft [#356](https://github.com/0x-copilot-dev/0x-copilot/pull/356) adds generic MCP decisions and immutable binding, but review found the stale-v1 guard weaknesses and a same-org non-owner can currently fall back to the mutable legacy stage.                                                                                                                                                                  | Bind the proposal and coordinator exclusively to an Artifact revision/content digest; retain generic owner-authorized decisions; deny non-owners before any legacy/migrated side effect; and fail closed on lookup error or any F-006 supersession across host-run changes. Do not add a draft-specific bypass or copy bytes at approval time.                                                                                                                                                                                                                                                  | **Release blocker — correction in progress.**           |
