...
| F-002 | The sandbox must be desktop and filesystem first; selected bytes must be sealed and verified before any provider receives them.                         | [D3 contract](prds/PRD-D3-sandbox-adapter.md), [desktop file-store migration](../../operations/desktop-file-store-migration.md), independent D3 draft review (2026-07-26): sealing now prevents mismatched bytes from reaching a provider, but a lost C1 pointer with retained history can still authorize an empty plan and queued context can disagree with the persisted run. | Retain the file-backed lifecycle/session/usage/cleanup namespace and C1/C3-only local mutation. Seal each selected input before upload; distinguish genuinely new workspace state from pointer/history corruption; and bind org/user/run/version exclusively from the verified persisted run before any snapshot/provider work. Non-file/Postgres history remains unavailable; do not add a fallback.                    | **Release blocker — correction in progress.**           |
...
| F-005 | D3 has a typed gateway/tool seam but no verified production worker composition, so the model-visible sandbox capability is correctly unavailable.       | Independent D3 draft review (2026-07-26): the candidate has a sealed bundle but lacks durable cleanup/reaper wiring, accepts in-memory stores through duck typing, and has no real worker patch-collector/provider composition.                                                                                                                                                  | Compose exactly one file-native `SandboxWorkerBundle` from verified C1/A2 snapshot authority, concrete trusted file-store identities, a durable restart-safe cleanup reaper, a real patch collector, and an attested provider gateway. The model-facing tool may call only the canonical coordinator path; direct `session_scope`/`aexecute`, PostgreSQL-history fallback, and automatic patch import remain prohibited. | **Release blocker — correction in progress.**           |
| F-006 | Draft-send has two competing sources of truth: it stages legacy mutable draft bytes rather than the selected immutable Artifact revision.               | Current `main` still stages `DraftRecord` through legacy `WriteStager`. Draft [#356](https://github.com/0x-copilot-dev/0x-copilot/pull/356) adds the generic owner-authorized MCP decision route and immutable binding, but independent review found its stale-v1 guard fails open on event-store error and misses a stage after a draft moves host run.                         | Bind the proposal and coordinator exclusively to an Artifact revision/content digest; retain the generic effect-decision path/UI; and fail closed on stale-v1 lookup error or any F-006 stage correlated to that draft across host-run changes. Do not add a draft-specific bypass or copy bytes at approval time.                                                                                                       | **Release blocker — correction in progress.**           |
