...
And on the desktop app the primary CTA leads somewhere the design does not have. "Connect
a tool" does not open a connect dialog; it flips to the Available tab
(`apps/desktop/renderer/destinationBinders.tsx:485` `onConnect={() => setFilter("available")}`).
**The desktop CTA is not dead** — the Available tab's cards are bound to a real broker
(`onOpenCatalogEntry={connect}` at `:486` → `CONNECTOR_CHANNELS.connect` → Electron main's
system-browser OAuth), so a desktop user _can_ connect a catalog app today. The gap is
narrower and exactly two things wide: there is **no catalog + permission modal**, so the
access decision is never presented at connect time; and there is **no custom-MCP add at
all**, a capability that exists only on web. Deleting the tab strip (D2) therefore removes
desktop's only working catalog path, which is why D2 and D4 must land in the same change.
...
| Desktop has **no connect flow** / "the desktop CTA does nothing"                      | `apps/desktop/renderer/destinationBinders.tsx:445-491`                                                                                                                                                                                            | **DISPUTED — the code wins.** `ConnectModal` is never mounted (CONFIRMED: the binder passes only `items/filter/onFilterChange/onConnect/onOpenCatalogEntry/onOpenApprovalSettings/onRetry`). But `onConnect={() => setFilter("available")}` (`:485`) reaches a **live** catalog tab and `onOpenCatalogEntry={connect}` (`:486`) invokes `CONNECTOR_CHANNELS.connect` (`apps/desktop/main/connectors/channels.ts:17`) → `apps/desktop/main/ipc/handlers.ts:406` → `apps/desktop/main/connectors/oauth-coordinator.ts:201,239`, i.e. real system-browser OAuth. Missing = the **modal (catalog + permission step)** and the **custom-MCP path**; the transport and the catalog both already work. Program README cross-cutting note 5 records the same correction. |
...
| Modal identity-tile chrome exists, is fed a text glyph, **and picks the wrong rung**  | `packages/chat-surface/src/settings/Modal.tsx:161-171`; `ConnectModal.tsx:307`                                                                                                                                                                    | CONFIRMED, and the chrome is **not** correct. `logoStyle` = 30×30, `--radius-md`, `backgroundColor: var(--color-surface-muted)` (#16161a = the design's `--panel2`, the _hover_ rung) and `color: var(--color-text-muted)`. Design `.modal__logo` (`copilot.css:2260-2271`) = 34×34, radius 8, `--panel3` / `--tx2` — which are exactly `--color-surface-elevated` / `--color-text-strong` (`styles.css:171,201`). Measured HIGH ×2 (`surfaces/tools/out/report-connect.md:29-30`). This is the program's dominant "the token exists, the call site picks the wrong one" pattern, and it uses README C10's ratified mapping. `ConnectModal` passes `logo={<span aria-hidden="true">◆</span>}` (`:307`).                                                          |
...
**Scrim / modal shell** (`copilot.css:2223-2232`, README **G10**) — the full declaration,
because four of its properties are measured and none is a token:
...
Live `settings/Modal.tsx:138-159` is `position: fixed`, `display: flex` + `alignItems`/
`justifyContent: center`, `padding: 16`, `zIndex: 1100`. Measured:
`report-connect.md:92-94` — `display grid → flex`, `justifyContent normal → center`,
`padding 22px → 16px` (all MEDIUM), plus the HIGH `backgroundColor` row at `:28` which is
PRD-01's token. `position` and `z-index` are not in the extractor's property set today, so
they are specified here but not gated by a report row.

## Architectural decision

Eight decisions. Six of them **consume things that already exist**; one deletes a feature;
one (D8) is pure geometry the README assigned here.
...
**`_shared/Row.tsx` is [PRD-08](PRD-08-activity-surface.md)'s file for the whole program
(README C9), and the stacking order on it is `08 → 09 → 11`.** PRD-08 lands the `trailing`
slot + always-reserved 16px, `iconTone`, the icon-slot `background:
var(--color-surface-elevated)`, `className="ui-list-row"`, `rowStyle.padding: "11px 14px"`
and the title weight absorbed from PRD-04; [PRD-09](PRD-09-chats-surface.md) then stacks
its `overflow` (⋯) slot. PRD-11 adds exactly two props to that **already-merged** file and
re-derives none of theirs. In particular this PRD does **not** set row padding, row hover,
the title weight, or the icon-slot background.
...
Two interactions with PRD-08's version of the file, both checked in code:

- At `iconSize={30}` the `AppIcon` tile (30px, `--radius-md`) exactly covers the slot, so
  PRD-08's `iconSlotStyle.background: var(--color-surface-elevated)` is never visible on a
  connector row and needs no per-call-site opt-out. This answers PRD-08's risk row "the
  tile background lands on rows whose icon is a full-bleed logo (Tools connector marks)".
- PRD-08's 15px glyph rule is scoped `.ui-list-row [data-testid="row-icon"] svg`
  (`PRD-08-activity-surface.md:452-455`). `AppIcon` never emits an inline `<svg>` — it
  renders a text glyph (`BRAND_GLYPHS[slug].symbol`), a letter, or an `<img>`
  (`packages/design-system/src/index.tsx:492-581`) — so that rule cannot shrink a
  connector tile.
...
stops existing. With no tab to switch to, `onConnect` can only mean "open the modal" on
both hosts.

**Sequencing constraint (not optional).** Desktop's Available tab is a _working_ catalog
today (`destinationBinders.tsx:485-486`; see Evidence). Deleting it without D4 in the same
change would remove desktop's only path to installing anything. **D2 and D4 land together
or neither lands.**
...
1. **design-system** gains `.ui-app-icon--tile` (`width/height: 1.875rem` = 30px,
   ...
   gains `size?: "sm" | "lg" | "tile"` (today `"sm" | "lg"`,
   `packages/design-system/src/index.tsx:508`), plus `tone?: "brand" | "neutral"` (default
   `"brand"`; `"neutral"` suppresses the inline brand surface set at `:530` and `:555` so the tile
   renders `--color-surface-elevated` / `--color-text-strong`).
   Checked before minting anything: there is no existing square/tile `.ui-app-icon`
   variant (`grep -n "ui-app-icon" packages/design-system/src/styles.css` → `:854, 869,
878, 883, 891, 895, 917`, all circles or modifiers of the circle), and **no new token is
   introduced** — the class consumes `--radius-md` (`:109`) and `--font-size-xs` (`:64`)
   that already exist. `--font-size-xs` is 12.48px against the design's 12px
   (`copilot.css:1611`); that residual 0.48px lands as a MEDIUM row, not a HIGH, and is not
   worth a new rung on a ladder PRD-01 §B explicitly refuses to extend.
...
`primaryButtonStyle` is untouched — repairing it for other surfaces is
[PRD-09](PRD-09-chats-surface.md)'s territory (it keeps the topbar), not a second fix here.

**The recipe has one known defect and it is not fixed here.** `.ui-button--sm` sets
`font-weight: var(--font-weight-medium)` (500) while `.ui-button--primary` sets only
`background` + `color`, so `--primary --sm` computes 500 where the design's
`.cbtn--pri` re-asserts 600 (`packages/design-system/src/styles.css:443-449` vs
`:462-465`; `copilot.css:491-496`). README **G9** assigns that one line to
[PRD-01](PRD-01-design-tokens.md) §E, which is a hard dependency of this PRD precisely
because the Tools CTA would otherwise inherit it. Do **not** patch the weight at this
call site.
...
### D8 — The modal shell geometry is this PRD's (README G10)

`report-connect.md:92-94` measures three shell rows nothing else owns:
`display grid → flex`, `justifyContent normal → center`, `padding 22px → 16px`. PRD-01
declared this "modal-shape, not token" and is right — it is a `CSSProperties` change in
`settings/Modal.tsx:138-159`, not a token. README **G10** assigns it here.

`scrimStyle` becomes `display: "grid"`, `placeItems: "center"` (dropping `alignItems` /
`justifyContent`), `padding: 22`. `position` stays `"fixed"` and `zIndex` stays `1100`:
the design's `position: absolute` / `z-index: 60` are values of a self-contained mock
window, `Modal` is a real app overlay, neither property is in the extractor's captured set
(`tools/design-parity/lib/extract-computed.js:22-55`), and changing them would be a
functional regression for zero measured parity. That reasoning is recorded once here
rather than as an `expectDivergence` the harness cannot raise.

The same file's `logoStyle` (`:161-171`) is retuned in the same change: 30→34px, and
`--color-surface-muted`/`--color-text-muted` → `--color-surface-elevated`/
`--color-text-strong` (README C10's mapping; closes the two HIGH rows at
`report-connect.md:29-30`).
...
(`access_mode` → [PRD-06](PRD-06-connector-access-mode.md)). Any implementation that
reaches for a migration has taken a wrong turn.

Verified on disk at the time of writing: `services/backend/migrations/` high-water mark is
**`0045`** (`0045_provider_api_keys_custom_endpoint.sql`) and
`services/ai-backend/migrations/` is **`0001`** (`0001_runtime_baseline.sql`), matching the
README's pre-assignment table. **PRD-11 claims no id** in either service, so it must not
appear in `MANIFEST.lock` and must not run `tools/check_migration_manifest.py --write`.
...
| File                                                         | Reason                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/destinations/_shared/Row.tsx`                           | Add `subFont` and `iconSize` props **only** — PRD-08 owns the file (README C9), PRD-09 stacks before us; order `08 → 09 → 11`. Do not re-derive their props.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `src/destinations/_shared/Row.test.tsx`                      | Cover both new props.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `src/destinations/connectors/ConnectorsDestination.tsx`      | **PRD-06 edits this file first (wave 0); PRD-11 second (wave 3) — README C6.** Rewrite the body onto `PageLead` + `SectionHeader` + `RowList` + `Row`; drop `PageHeader`/`FilterTabs`/`CardGrid`/`CatalogCard`; drop `filter`/`counts`/`onOpenCatalogEntry`; render the default `AppIcon` tile; restore the design's lead copy (the existing `tools-policy-note` link becomes the `PageLead` link); move the status chip to error/expired/disconnected only. **Do not touch `accessPort`, the optimistic overlay or the access-mode error banner — those are PRD-06's and arrive before us.** |
...
| `src/settings/Modal.tsx:138-159`                             | D8 shell geometry (README G10): `display: "grid"` + `placeItems: "center"`, `padding: 22`. Drop PRD-01's `--color-scrim` fallback string (PRD-01 lands the token + `backdropFilter`). Keep `position: "fixed"` / `zIndex: 1100`.                                                                                                                                                                                                                                                                                                                                                              |
| `src/settings/Modal.tsx:161-171`                             | `logoStyle` 30→34px; `--color-surface-muted` → `--color-surface-elevated`; `--color-text-muted` → `--color-text-strong` (README C10).                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| File                             | Reason                                                                                                                                                                                                                                                                                                                                                                                                                      |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `surfaces/tools/anchors.json`    | Repoint live selectors at the row-list DOM; `default.section.head` and `default.row.logo` must resolve; add `expectDivergence` (the key `lib/compare.mjs:172` reads — not `expected-divergence`) for the 8/6-vs-7/5 radius and the modal `StepDots`. Also correct the now-stale `default.seg.selected` note at `:103` claiming "boxShadow is not in the extractor's property set" — it is, at `lib/extract-computed.js:48`. |
| `lib/render-live-tools.test.tsx` | Re-render both states against the new component tree.                                                                                                                                                                                                                                                                                                                                                                       |

## Non-goals

- **`access_mode` on the wire, and the `ConnectorAccessPort`.**
  [PRD-06](PRD-06-connector-access-mode.md) owns `ConnectorResponseModel.access_mode`, the
  PATCH route, its authorization rule, all three enforcement gates, and the
  `accessPort` prop that replaces `onSetAccessMode`. This PRD renders whatever PRD-06
  delivers. **The `?? "off"` fallback at `ConnectorsDestination.tsx:338` is deleted by
  PRD-06, not kept** (README **C4**) — once the wire type makes the field required there
  is no fallback to keep. An earlier draft of this PRD claimed to preserve it; that clause
  is struck.
- **`_shared/Row.tsx`'s trailing slot, `iconTone`, icon-slot background, row padding,
  hover, title weight, `.ui-list-row`, and the ⋯ overflow slot.**
  [PRD-08](PRD-08-activity-surface.md) owns the file (README **C9**) and
  [PRD-09](PRD-09-chats-surface.md) stacks before us. PRD-11 adds `subFont` + `iconSize`
  and nothing else.
- **Shell/destination binding totality.** [PRD-03](PRD-03-host-binder-contract.md)
  declares `ConnectorsDestination` **out of scope** (README **C6**) and applies
  binding-totality to the shell + projects only; its `connect` union and its nullable
  `onSetAccessMode` are gone. So the connectors prop surface has exactly two owners:
  PRD-06 (`accessPort`) and PRD-11 (everything else, including mounting `ConnectModal` on
  both hosts per README **C5**). `renderIcon` survives as an optional host override; PRD-11
  changes what the destination does when it is _absent_.
- **`--color-scrim` / `--blur-scrim` / `--font-size-sm` retune / `.ui-button--primary`
  weight (G9).** [PRD-01](PRD-01-design-tokens.md) owns all four; this PRD only stops the
  local `--color-scrim` fallback in `settings/Modal.tsx` and consumes the fixed button
  recipe. The scrim's `backdrop-filter` is PRD-01's line, not D8's.
- **`PageHeader`'s hand-rolled primary button.** Tools stops mounting `PageHeader`; other
  surfaces still use it. Fixing it there belongs to PRD-09, which keeps the topbar.
...
| Deleting the "Available" tab removes the only non-modal path to the catalog — **on desktop it is a working path today** (`destinationBinders.tsx:485-486`).      | `ConnectorsRoute.test.tsx:423-538` (`describe("ConnectorsRoute connect flow")`) and the new desktop binder test both assert the modal path reaches the catalog. D2 and D4 must land in one change (see D2).                                                                                                                                                      | If the modal path regresses, users cannot install anything — this is the highest-value test in the set, run it first. |
| `useConnectFlow` extraction changes web behaviour that currently passes.                                                                                         | The four named suites in `apps/frontend/src/features/connectors/__tests__/ConnectorsRoute.test.tsx` — `"ConnectorsRoute connect flow"` (`:423`), `"ConnectorsRoute custom server add"` (`:540`), `"ConnectorsRoute SSE"` (`:225`), `"ConnectorsRoute reconnect"` (`:703`) — must all still be present and passing (DoD 9).                                       | Revert the hook file and re-inline; the modal contract is unchanged either way.                                       |
...
| `AppIcon` gaining props regresses ~15 existing call sites.                                                                                                       | Both new props are optional with today's behaviour as the default. `packages/design-system` has **no test root** — only a `typecheck` script (README DoD-Q3) — so the guard is `npm run typecheck --workspace @0x-copilot/design-system` plus the in-tree consumer tests `packages/chat-surface/src/citations/SourceFavicon.test.tsx` and `MessageSourcesStrip`. | Revert `styles.css` + `index.tsx` independently of the surface work.                                                  |
...
| **Lost edits in the program's hottest files.** `destinationBinders.tsx` has eight claimants; `Row.tsx` four; `styles.css` five; `ConnectorsDestination.tsx` two. | Land strictly in the README's per-file order: `Row.tsx` 08 → 09 → **11**; `styles.css` 01 → 02 → 08 → **11** → 10; `ConnectorsDestination.tsx` 06 → **11**; `destinationBinders.tsx` per wave (PRD-11 is wave 3, after 09). One merge owner per wave.                                                                                                            | Per-file; each seam is independently revertible.                                                                      |
| Rollback of the whole PRD.                                                                                                                                       | Every change is additive-or-deletive within four packages and touches no schema, no migration and no route.                                                                                                                                                                                                                                                      | `git revert` the PRD's commits; PRD-01/06/08/09 remain independently valid.                                           |

## Definition of Done

1. `grep -rn "CardGrid\|PageHeader\|FilterTabs" packages/chat-surface/src/destinations/connectors --include='*.tsx' --include='*.ts' | grep -v '/webhooks/' | wc -l` prints **0**. **The `/webhooks/` exclusion is deliberate, not a loophole:** `destinations/connectors/webhooks/WebhooksDestination.tsx:18,51` imports `PageHeader` and that subtree is an explicit non-goal, so an unfiltered directory grep would be a gate this PRD cannot pass. The form also tolerates `ConnectorsPanel.tsx` being deleted outright.
...
5. **Second design value pinned:** `AccessModeSegment.tsx`'s group background resolves to `var(--color-surface)` (= `#111114` = `--panel`, `copilot.css:10`) and the selected item's to `var(--color-surface-elevated)` (= `#1d1d23` = `--panel3`, `copilot.css:12`). Verify with one command: `grep -c -- "--color-bg" packages/chat-surface/src/destinations/connectors/AccessModeSegment.tsx` prints **0** (this also covers `--color-bg-elevated`), and `grep -c -- "--color-surface-elevated" …/AccessModeSegment.tsx` prints **≥1**.
6. `grep -c "boxShadow" packages/chat-surface/src/destinations/connectors/AccessModeSegment.tsx` prints **0**, and `AccessModeSegment.test.tsx` contains an assertion of the form `expect(getComputedStyle(getByTestId("access-mode-option-read_act")).fontWeight).toBe(getComputedStyle(getByTestId("access-mode-option-off")).fontWeight)` (test ids exist today at `AccessModeSegment.tsx:131`). The harness now captures `boxShadow` (`tools/design-parity/lib/extract-computed.js:48`), so the ring is additionally visible in the report.
7. `npm test --workspace @0x-copilot/chat-surface -- src/destinations/connectors src/destinations/_shared` exits 0, and the run includes `src/destinations/connectors/useConnectFlow.test.ts`.
8. `npm test --workspace @0x-copilot/desktop -- destinationBinders` exits 0, with `apps/desktop/renderer/destinationBinders.test.tsx` asserting (a) clicking the Tools destination's "Connect a tool" CTA renders `[data-testid="settings-modal"]` whose title is `"Connect a tool"`, and (b) submitting the custom-server form calls the injected port's `addCustomServer` exactly once. **This is the regression guard that fails on `main`** — desktop mounts no modal today (`destinationBinders.tsx:479-490`).
9. `npm test --workspace @0x-copilot/frontend -- ConnectorsRoute` exits 0, and `grep -c 'describe("ConnectorsRoute \(connect flow\|custom server add\|SSE\|reconnect\)"' apps/frontend/src/features/connectors/__tests__/ConnectorsRoute.test.tsx` prints **4** — the four suites that guard the extracted orchestration are still present, not deleted along with the aside.
10. `grep -c "mcp-card__icon" apps/frontend/src/features/connectors/mcp/mcp-wizard.css` prints **0**, and `grep -c 'size="tile"' apps/frontend/src/features/connectors/mcp/McpOverlay.tsx` prints **2**.
11. `npm run typecheck --workspace @0x-copilot/frontend && npm run typecheck --workspace @0x-copilot/desktop && npm run typecheck --workspace @0x-copilot/chat-surface && npm run typecheck --workspace @0x-copilot/design-system` exits 0. (`packages/design-system` has only a `typecheck` script — no test root — per README DoD-Q3; item 3 covers its rendering behaviour through the chat-surface suite.)
12. `npm run lint --workspace @0x-copilot/chat-surface` exits 0 — proving `useConnectFlow` contains no bare `window` / `fetch` / `localStorage` (the substrate ban).
13. **Parity re-run, absolute target** (procedure: `tools/design-parity/SKILL.md` steps 2-4; live fixtures via `node_modules/.bin/vitest run --config tools/design-parity/vitest.config.mjs`, then `node tools/design-parity/lib/compare.mjs surfaces/tools/out/design-default.json surfaces/tools/out/live-default.json --anchors surfaces/tools/anchors.json --out surfaces/tools/out/report-default.md --state default`): in the regenerated `tools/design-parity/surfaces/tools/out/report-default.md`, `awk '/^## 🔴 HIGH/,/^## 🟠/' report-default.md | grep -cE '\| (Section header|List|Row|Permission control) +\|'` prints **0**. No count is frozen: the pre-change artefact is `tools/design-parity/surfaces/tools/out/report-default.md` **as committed on this PR's merge base** (`git show $(git merge-base HEAD origin/main):tools/design-parity/surfaces/tools/out/report-default.md`), and the movement is read as the diff against it. (README **DoD-Q12**. For the record, the merge-base artefact reads `HIGH 14`, not the 15 an earlier draft of this PRD asserted — and per README cross-cutting note 6 the harness's own `compare.mjs`/`extract-computed.js` changes already moved HIGH totals program-wide, which is why a frozen number is not a gate.)
14. In that regenerated report, `default.section.head` and `default.row.logo` no longer appear as `missing-in-live`, and `live.filter.tabs`, `live.page.header.title`, `live.status.pill` and `live.connectors.panel` no longer appear as `extra-in-live`. Verify: `grep -cE 'default\.(section\.head|row\.logo).*missing-in-live|live\.(filter\.tabs|page\.header\.title|status\.pill|connectors\.panel).*extra-in-live' tools/design-parity/surfaces/tools/out/report-default.md` prints **0**.
15. `node -e 'const a=require("./tools/design-parity/surfaces/tools/anchors.json");const need=["default.seg","default.seg.selected","default.seg.unselected","default.seg.read.selected","live.connect.stepdots"];const ok=need.every(l=>a.elements.some(e=>e.label===l&&typeof e.expectDivergence==="string"&&e.expectDivergence.length>0));process.exit(ok?0:1)'` exits 0 — each carrying its reason string (token-ladder quantization for the 8/6-vs-7/5 radius; multi-step OAuth needs a progress affordance the instant mock does not). The file's shape is `{elements:[{label,…}]}` and the key the comparator actually reads is **`expectDivergence`** (`tools/design-parity/lib/compare.mjs:172`) — not `expected-divergence`.
16. In the regenerated `tools/design-parity/surfaces/tools/out/report-connect.md`, `awk '/^## 🔴 HIGH/,/^## 🟠/' report-connect.md | grep -cE '\| Catalog +\|'` prints **0** — i.e. `connect.catalog.row.logo` renders a real per-slug tile (`--panel3` / `--tx2` / 11px), not `◆`.
17. `grep -c "color-scrim, " packages/chat-surface/src/settings/Modal.tsx` prints **0** (the local fallback is gone once PRD-01's token lands).
18. **D8 / README G10 — modal shell geometry.** `grep -cE '"grid"|placeItems|padding: 22' packages/chat-surface/src/settings/Modal.tsx` prints **3**, and in the regenerated `report-connect.md` the rows `connect.scrim × display`, `connect.scrim × justifyContent` and `connect.scrim × padding` are absent: `grep -c 'connect\.scrim.*| \(display\|justifyContent\|padding\) ' tools/design-parity/surfaces/tools/out/report-connect.md` prints **0**.
19. **D8 / README C10 — modal logo rung.** `grep -A11 "const logoStyle" packages/chat-surface/src/settings/Modal.tsx` contains `width: 34`, `height: 34`, `var(--color-surface-elevated)` and `var(--color-text-strong)`, and the two HIGH rows `connect.modal.logo × color` / `connect.modal.logo × backgroundColor` (present at `report-connect.md:29-30` on the merge base) are absent from the regenerated report: `grep -c 'connect\.modal\.logo.*| \(color\|backgroundColor\) ' tools/design-parity/surfaces/tools/out/report-connect.md` prints **0**.

## Dependencies

PRD-11 is **wave 3**, landing after PRD-09 and in parallel with PRD-12 (README batch 3b —
the only file both touch is the `src/index.ts` barrel).

**Must land first**

- **[PRD-01](PRD-01-design-tokens.md) (design tokens)** — `--color-scrim` / `--blur-scrim` for the modal scrim (DoD 17), the `.ui-button--primary` weight fix (README **G9**) the Tools CTA inherits, the `SectionHeader` → `.ui-mono-caps` migration at `--font-size-mono-9-5` that gives the `Connected · N` eyebrow its 9.5px (README **C13**: PRD-01 applies the recipe to the **label element**, not the wrapper — PRD-11 mints **no** font token and hand-rolls no eyebrow), and the `--font-size-sm` → 13px retune that most of the ≤2px type deltas on this surface ride on. File order on `styles.css`: 01 → 02 → 08 → **11** → 10.
- **[PRD-06](PRD-06-connector-access-mode.md) (access-mode backend)** — `ConnectorResponseModel.access_mode` on the wire, the PATCH route, the `ConnectorAccessPort`, and the deletion of the `?? "off"` fallback (README **C4**). Without it the segment renders a permanent `off` for every connector and D5's visual work is unverifiable end-to-end. **PRD-06 edits `ConnectorsDestination.tsx` before us; order `06 → 11`** (README **C6**).
- **[PRD-08](PRD-08-activity-surface.md) (activity surface)** — **owns `_shared/Row.tsx`** for the whole program (README **C9**): `trailing` + the always-reserved 16px, `iconTone`, the icon-slot background, `.ui-list-row`, `11px 14px` padding, and PRD-04's absorbed title weight. PRD-11 stacks `subFont`/`iconSize` on the merged file.
- **[PRD-09](PRD-09-chats-surface.md) (chats surface)** — stacks its `overflow` (⋯) slot on `Row.tsx` between PRD-08 and us (`08 → 09 → 11`), and owns the topbar/full-bleed split. Tools is not full-bleed, so the split is order-only for this PRD.

**Ordering only — corrected**

- **[PRD-03](PRD-03-host-binder-contract.md) (host bindings)** is listed as a dependency in the README's PRD table and lands in wave 1, two waves ahead of us — but it **supplies none of this surface's props**. An earlier draft of this PRD listed it as a hard dependency for "`onSetAccessMode` / `ConnectModal` prop plumbing"; both halves are now wrong. README **C5** gives `ConnectModal` — on **both** hosts — to PRD-11, because desktop's renderer is denied `window.open` (`apps/desktop/renderer/onboarding/firstRunConnectorsPort.ts:17-21`) and authorization is therefore a genuine host capability a destination cannot own; and README **C6** puts `ConnectorsDestination` **out of PRD-03's scope entirely**, so its `connect` union and its nullable `onSetAccessMode` are gone (`onSetAccessMode` is deleted by PRD-06). What PRD-03 does give us is the settled shell/destination binding contract that `destinationBinders.tsx` is written against. There is therefore **no "hold D4 until PRD-03 lands"** clause: D4 ships with the rest of this PRD.

**Soft ordering**

- **[PRD-10](PRD-10-projects-surface.md) (projects)** also consumes `_shared/Row` and lands after us on `styles.css`. PRD-11 **owns** the `subFont` and `iconSize` additions to `Row`; siblings consume them and must not re-add equivalents.
