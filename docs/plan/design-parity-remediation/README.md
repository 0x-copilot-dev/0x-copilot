...
### Outstanding items — closed 2026-07-23 (orchestrator)

All seven O-items are resolved. Verified after the fact, not assumed:

| #   | Resolution                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| O1  | `baseline-failures.txt` **created and committed** as the Wave-0 prologue. Captured on a tree byte-identical to `origin/main` (`git diff origin/main...HEAD -- packages/ apps/ services/` is empty). Result: chat-surface 2440, frontend 1355, desktop 1063+1 todo all green; api-types has ONE pre-existing failure — `packages/api-types/src/brands.test.ts` is type-only, so vitest finds no runtime suite and fails the FILE, exiting 1 despite 44/44 passing. Deliberately not fixed here: it would move the baseline the six DoD items compare against. |
| O2  | **PRD-10 owns** the `ActivityDestination` → `<Page>` swap (addendum + 3 DoD items). PRD-10 ships the primitive, and a primitive whose first sibling caller is never migrated is just more dead code.                                                                                                                                                                                                                                                                                                                                                         |
| O3  | **PRD-09 accepts** the `surfaces/chats/anchors.json:41-42` `sect.*` retarget (addendum + DoD item), in a file it already edits.                                                                                                                                                                                                                                                                                                                                                                                                                              |
| O4  | PRD-06 Dependencies rewritten: Wave 0; the false "touches no file the siblings own" claim is retracted and now names C3/C4/C6; "the Tools styling PRD" resolved to **PRD-11**, with the `06 → 11` order stated. (Its reconciling agent stalled mid-stream — the one agent failure in that run.)                                                                                                                                                                                                                                                              |
| O5  | PRD-10's `--color-bg` clause **deleted**. PRD-01 decision F ruled G3 INVALID: `--color-bg` already equals the design's `--ink`; `#050506` is the mock's stage and appears in zero report rows.                                                                                                                                                                                                                                                                                                                                                               |
| O6  | PRD-09 DoD 6 now invokes `../../tools/check_migration_manifest.py` — the tool is at the repo root; `services/ai-backend/tools/` does not exist.                                                                                                                                                                                                                                                                                                                                                                                                              |
| O7  | Dangling `PRD-14` routes rewritten to name PRD-12 D9. PRD-07's ai-backend hot-file order corrected to `05 → 08 → 07 → 09 → 12`. **Correction to the audit:** O7 claimed C2 gave the `railIdentity` shape to PRD-03 and that PRD-12 `:447` was therefore wrong. C2's ruling column actually says **PRD-12's shape wins**, bound once by PRD-03. The stale-scope-row symptom was real (`:447` contradicted PRD-12's own `:313`/`:494`/`:502`, which all defer the binding), so the row was corrected — but for the opposite reason the audit gave.             |
