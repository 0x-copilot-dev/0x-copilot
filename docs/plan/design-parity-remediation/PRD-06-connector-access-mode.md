...
| Desktop binds every callback **except** the access-mode one  | `apps/desktop/renderer/destinationBinders.tsx:480-489`                            | `<ConnectorsDestination items filter onFilterChange onConnect={() => setFilter("available")} onOpenCatalogEntry={connect} onOpenApprovalSettings onRetry />` — `onSetAccessMode` is the **only** absent prop, so `ConnectorsDestination.tsx:339-343` passes `undefined` and the segment is inert. **Correction to the audit brief:** desktop's connector CTA is _not_ dead — `onConnect` filters to the catalog and `onOpenCatalogEntry` reaches real OAuth over IPC. The desktop gap this PRD closes is exactly one prop wide. |
...
Three line ranges were off by a few lines (noted above); three material facts were **missing** from the brief and are added: the durable DDL lives in `migrations/0044_connectors.sql` (not just the module `schema.sql`), the parity harness cannot see this bug, and `proxy_internal_rpc` is a single live chokepoint that makes real enforcement cheap.

One brief claim **is** refuted, and the code wins: "the desktop connector CTA does nothing" is false (`destinationBinders.tsx:485-486`). The desktop defect is scoped to `onSetAccessMode` alone.

**Migration high-water mark, verified on disk** (`ls services/backend/migrations`, `ls services/ai-backend/migrations`): `services/backend` tops out at `0045_provider_api_keys_custom_endpoint.sql`, so `0046` is this PRD's id per the program README's C18 reassignment. `services/ai-backend` has only `0001_runtime_baseline.sql` — **this PRD adds no ai-backend migration**; `0002`/`0003`/`0004` belong to PRD-05/PRD-07/PRD-09.
...
plus MEDIUM: group radius `7px → 8px`, item radius `5px → 6px`, item padding `5px 12px → 4px 10px`, selected weight `500 → 600`. Those come from `AccessModeSegment.tsx:141-176` using `--color-bg` / `--color-bg-elevated` / `--radius-md` / `--radius-sm` instead of panel-family tokens. **Those style rows are assigned to [PRD-11 (Tools surface)](PRD-11-tools-surface.md), not this one** — but note they were only measurable because the harness fakes a selected state. See Non-goals.

**Harness state, verified.** `tools/design-parity/lib/extract-computed.js:22-50` now also captures `boxShadow`, `backdropFilter`, `transition` and `textDecorationLine`, and `lib/compare.mjs:53-64` classifies all of them **MEDIUM** (`BOX` set). The committed `report-default.json` predates that capture (`prop: "boxShadow"` appears zero times in it), so re-extracting will add MEDIUM rows — notably PRD-11's accent selection ring — but **cannot** change the Permission-control HIGH set. That set is and stays exactly the three `backgroundColor` rows above, which is what DoD 17 pins. No token in this PRD is new: `--panel` `#111114` and `--panel3` `#1d1d23` already have byte-identical design-system equivalents — `--color-surface` (`packages/design-system/src/styles.css:170`) and `--color-surface-elevated` (`:201`), both verified. (`--color-surface-muted` at `:171` is `#16161a` = the design's `--panel2`/hover rung and is **not** the segment colour.) PRD-11 owns repointing `AccessModeSegment.tsx` to them; this PRD adds no token.
...
Migration **`0046_connector_access_mode.sql`** (+ `.rollback.sql`, + regenerated `MANIFEST.lock`). This id is the program README's C18 assignment and matches the on-disk high-water mark (`0045`) verified in Evidence; PRD-03's competing `0046_connector_access_mode.sql` is deleted per C3, and PRD-07 takes `0047`.

The durable DDL and the module mirror must be written **together**: `services/backend/src/backend_app/connectors/schema.sql` is what fresh installs execute, and a migration/mirror divergence has previously shipped a 500 on fresh installs (the `0043_projects` incident). DoD 6 checks both files, not just the migration.
...
Service method `ConnectorsService.set_access_mode(...)` mirrors `patch_scopes`: `_authorize_write` → no-op return when unchanged (no audit noise) → `model_copy` → inside `store.transaction(org_id=tenant_id)` write the row and append **one** `ConnectorAuditRecord` with `action="connector.access_mode_changed"`, `before_state`/`after_state` via `_safe_dump`, and `correlation_id=f"{previous}->{next}"`. The audit action string is `connector.access_mode_changed` — PRD-03's draft named it `connector.access_mode_set`; that PRD's backend section is deleted by C3, so `_changed` is the single spelling. **Idempotency is explicit:** a PATCH whose value equals the stored value still returns `200` with the unchanged connector row and writes **zero** audit rows (DoD 3). That audit row rides the existing per-tenant HMAC hash chain (`store.py:726+`) — so "who changed this connector's authority, when, from what to what" is tamper-evident and exportable through the existing `GET /v1/connectors/{id}/audit`.
...
**Ownership of `ConnectorsDestination.tsx` (program README, C4/C5/C6).** Three PRDs wanted this file; the register settles it:

- **This PRD edits it first** (wave 0), then [PRD-11](PRD-11-tools-surface.md) (wave 3) — order `06 → 11`. [PRD-03](PRD-03-host-binder-contract.md) declares the destination **out of scope** and applies binding-totality to the shell + projects only, so its `connect` union and its nullable `onSetAccessMode` are gone.
- **The `?? "off"` fallback is deleted here** (C4). PRD-11's Non-goals previously claimed it "keeps the existing `?? \"off\"` least-privilege default"; that clause is struck. There is no fallback to keep once the wire type makes the field required.
- **`ConnectModal` and the connect flow are PRD-11's**, not this PRD's and not PRD-03's (C5) — desktop's renderer is denied `window.open`, so authorization is a genuine host capability. This PRD adds `accessPort` and nothing else to the destination's prop surface.
- `AccessModeSegment.test.tsx` is touched by both: this PRD lands the option-tuple assertions (DoD 16) first; PRD-11 adds its neutral-selection assertions to the same file afterwards. Neither replaces the other's tests.
...
- **The segment's computed-style deltas** (`--panel`/`--panel3` vs `--color-bg`/`--color-bg-elevated`, `7px/5px` radii, `5px 12px` padding, weight 500, and the accent selection ring at `AccessModeSegment.tsx:172`). Real, measured, and owned by **[PRD-11 (Tools surface)](PRD-11-tools-surface.md)** D5. This PRD must not touch `AccessModeSegment.tsx`'s style objects.
- **The connect flow, the catalog/permission modal and custom-MCP add** — [PRD-11](PRD-11-tools-surface.md) (C5), including the modal-shell geometry from `report-connect.md:94-97` (README G10). This PRD does not add or remove any connect-related prop.
...
6. **Migration and module mirror agree** (absorbed from PRD-03, whose backend section C3 deletes). This command prints `2`:

   ```bash
   grep -lF "access_mode TEXT NOT NULL DEFAULT 'read'" \
     services/backend/migrations/0046_connector_access_mode.sql \
     services/backend/src/backend_app/connectors/schema.sql | wc -l
   ```

   and this one also prints `2` (same two files):

   ```bash
   grep -lF "CHECK (access_mode IN ('read', 'read_act', 'off'))" \
     services/backend/migrations/0046_connector_access_mode.sql \
     services/backend/src/backend_app/connectors/schema.sql | wc -l
   ```
...
8. **Enforcement — `off`:** `cd services/backend && .venv/bin/python -m pytest tests/unit/connectors/test_access_mode_enforcement.py` (new file) exits 0, containing `test_proxy_internal_rpc_denies_off_connector`, which asserts `McpRegistryService.proxy_internal_rpc` raises the gate exception (mapped to `403 connector_access_off`) for a server whose connector row is `access_mode="off"`, **and** that the stubbed `token_vault.decrypt` recorded zero calls.
9. **Enforcement — `read`:** the same file contains `test_proxy_internal_rpc_read_mode_matrix`, asserting in one parametrised test that under `access_mode="read"` `proxy_internal_rpc` allows a `tools/list` envelope, allows a `tools/call` whose target tool advertises `annotations.readOnlyHint: true`, and raises for a `tools/call` whose target tool has **no** `annotations` key (fail-closed).
10. **Enforcement — no regression:** the same file contains `test_proxy_internal_rpc_allows_unjoined_server`, asserting every envelope in item 9 is allowed when no connector row joins the MCP server.
...
16. **Design value pinned numerically:** `packages/chat-surface/src/destinations/connectors/AccessModeSegment.test.tsx` asserts the radiogroup renders exactly **3** radios whose accessible names are, in order, `"Read"`, `"Read & act"`, `"Off"` — matching `tools/design-parity/design-kit/app-v3/copilot-app.jsx:139-141` `[["read","Read"],["act","Read & act"],["off","Off"]]` — and that exactly one has `aria-checked="true"`.
17. **Harness fixture is wire-shaped** (replaces the old "attributable to" wording; DoD-Q6). `tools/design-parity/lib/render-live-tools.test.tsx` types its six-row fixture as `ReadonlyArray<Connector>` and supplies `access_mode` per row from the design's own mix — five `"read_act"`, one `"read"` on `conn_github`, matching `copilot-data.jsx:511-551`. Since item 14 makes the field required, a fixture that omits it is a typecheck failure rather than a silently-`"off"` render. Checkable:

    ```bash
    grep -c 'access_mode: "read_act"' tools/design-parity/lib/render-live-tools.test.tsx   # prints 5
    grep -c 'access_mode: "read",' tools/design-parity/lib/render-live-tools.test.tsx      # prints 1
    ```

18. **Parity report regenerated and pinned** (DoD-Q6, split (a)/(b)). Re-render and re-compare per `tools/design-parity/SKILL.md` — `node_modules/.bin/vitest run --config tools/design-parity/vitest.config.mjs` exits 0, then re-extract and re-compare — and commit `tools/design-parity/surfaces/tools/out/`. Then:

    (a) **No anchor in the Permission-control group is unresolvable in live** — i.e. the selection is real markup, not a faked `data-value`. This prints `[]`:

    ```bash
    python3 -c "import json;d=json.load(open('tools/design-parity/surfaces/tools/out/report-default.json'));print([f['label'] for f in d['findings'] if f['group']=='Permission control' and f['kind']!='style'])"
    ```

    (b) **The remaining HIGH rows in that group are exactly the three background rows**, by label — all three owned by [PRD-11](PRD-11-tools-surface.md), none by this PRD. This prints the list below verbatim:

    ```bash
    python3 -c "import json;d=json.load(open('tools/design-parity/surfaces/tools/out/report-default.json'));print(sorted((f['label'],f['prop']) for f in d['findings'] if f['group']=='Permission control' and f['severity']=='high'))"
    # [('default.seg', 'backgroundColor'), ('default.seg.read.selected', 'backgroundColor'), ('default.seg.selected', 'backgroundColor')]
    ```

    Note on keys: the report JSON's fields are `label` / `prop` / `group` / `severity` / `kind` (`report-default.json`), not `anchor` / `property`; and `data-value` is never a compared property — `lib/extract-computed.js:22-50` compares computed styles only, and the seg anchors select by `data-testid` (`surfaces/tools/anchors.json:94-119`). (a) is the mechanical form of "the selected state is not faked". No absolute HIGH/MEDIUM totals are frozen here — re-extraction now also captures `boxShadow`/`backdropFilter`/`transition`/`textDecorationLine`, all MEDIUM (`lib/compare.mjs:53-64`), so totals will move.

19. `npm run typecheck --workspace @0x-copilot/frontend` and `npm run typecheck --workspace @0x-copilot/desktop` both pass.
