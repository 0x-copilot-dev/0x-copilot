...
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| **A1**       | Token/type parity: pin base ~12.5–13px size; add `--color-accent-line` (+ violet if needed); purge stale atlas-orange `var()` fallbacks                                                                                           | ⬜     |
| **A2**       | Web converges onto SSOT pages (`LocalModelsPort` + mount `LocalModelsPage`/`ModelBehaviorPage` on web); retire legacy `sections/{LocalModels,ModelAndBehavior,ProviderKeys}.tsx` + `providerKeysApi.ts`; slug↔page invariant test | ⬜     |
