...
| **B3**       | Provider keys polish. Keychain-copy reconciliation ✅ already on main (`PROVIDER_KEYS_KEYCHAIN_NOTE` = "…local vault…"). **Deferred (after C/D):** real brand logos (currently a first-letter glyph — visible polish) + custom OpenAI-compatible endpoint (D-2, a larger power-user feature). Sequenced after the broken-core waves.                                                                                                    | 🟡     |
...
- **B1+B2** feat(provider-keys): persist per-provider default_model on save; symmetric modelChips
