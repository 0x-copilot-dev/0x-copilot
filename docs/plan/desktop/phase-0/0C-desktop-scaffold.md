...
- Root `tsconfig.base.json` — verified: no explicit `paths` for `@enterprise-search/*`; npm workspaces handles module resolution. No edit needed.
...
- `Transport` from `@enterprise-search/chat-transport` — renderer-side IPC transport will implement it in Phase 1C.
- `ChatShell` (and the rest of the shell surface) from `@enterprise-search/chat-surface` — Phase 1B / 1A wire it through `renderer/bootstrap.tsx`.
- `SurfaceRegistry` + `registerAll()` from `@enterprise-search/surface-renderers` — Phase 4 / Phase 1A bootstrap call.
...
- `@enterprise-search/chat-surface` — `0.1.0`
- `@enterprise-search/chat-transport` — `0.1.0`
- `@enterprise-search/surface-renderers` — `0.1.0`
...
- [x] All FRs met
- [x] `npm install` at repo root succeeds and resolves `@enterprise-search/desktop`
- [x] `npm run typecheck --workspace @enterprise-search/desktop` exits 0
