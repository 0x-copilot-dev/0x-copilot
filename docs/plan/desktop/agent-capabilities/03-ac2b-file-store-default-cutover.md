...
- N4. ~~Making `append_events_batch` transactional under `file`.~~ **Resolved** (follow-up, post-cutover): the batch is now crash-atomic under `file` — one fsynced write per stream, all-or-nothing. See NFR-6.
...
- **NFR-6 (Event-batch durability, atomic — parity with Postgres).** `append_events_batch` under `file` is crash-atomic, matching Postgres's single transaction. The whole batch's monotonic `sequence_no`s are assigned up front, then every line is persisted through ONE fsynced append per target stream (`JsonlIo.append_lines`: a single joined `write` + one `fsync`) _before_ the in-memory materialized view and catalog index are advanced. A crash during the durable write therefore leaves either **all** of the batch's events on disk or **none** — never a fsynced proper prefix (the "partial coalesced flush" the earlier per-event loop could leave, one committed line per fsync). Coalesced `MODEL_DELTA` batches are all main-stream (`task_id is None`) → a single `open+write+fsync`; the per-stream grouping only matters if a producer ever mixes per-subagent `task_id` events into one batch, and even then each stream is written all-or-nothing. Covered by `tests/unit/runtime_adapters/file/test_batch_append_atomicity.py`.
