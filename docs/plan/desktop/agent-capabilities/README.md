...
| AC6  | [Monty interpreter/code mode](06-ac6-monty-code-mode.md)                                | AI execution/capabilities                   | AC2, AC4, and the AC3a in-process durable loop (Monty checkpoints reuse AC2-light + `SqliteSaver`; **AC3b not required**); **direct-path policy engine (`runtime_gate` wired) — see AC6** | **Planned — in scope** (multi-PR: interpreter foundation → policy-engine prereq → parity)                                       | None                                                                    |
| AC7  | [Remote sandbox execution](07-ac7-remote-sandbox-execution.md)                          | AI sandbox adapters                         | AC4, AC5                                                                                                                                                                                  | **Planned — in scope** (multi-PR: provider foundation → egress/secrets → lifecycle/patch-apply)                                 | None                                                                    |
| AC8  | [Agentic browser runtime](08-ac8-agentic-browser.md)                                    | Desktop browser worker, AI MCP/capabilities | AC4, AC5, AC9                                                                                                                                                                             | **Planned — in scope** (multi-PR: supervised-worker + read-only foundation → downloads/uploads/side-effects → profiles/consent) | None                                                                    |
...
- **AC6 — Monty code mode, in scope, built-in-first (Wave 3):** the engine is the DeepAgents/Monty `InterpreterPort`; we add only the enforcement layer. **(1) Interpreter foundation** — `InterpreterPort` + contracts + Monty adapter + isolation spike + pure-compute-only `run_code_mode`; **(2) policy-engine prereq** — wire the four-mode (`auto`/`ask`/`require`/`block`) direct-path tool policy (`runtime_gate.py`'s `ToolUsePolicyGate` is currently unwired dead code — see AC6) so `PolicyToolInvoker` has a real contract to mirror, plus the interrupt-from-tool-node seam; **(3) parity** — every external function through the one `PolicyToolInvoker`, approval/budget/citation/audit parity tests, checkpoint/resume. ~4–5 PRs.
- **AC7 — remote sandbox, in scope, built-in-first (Wave 3):** the engine is DeepAgents `SandboxBackendProtocol` (initial provider LangSmith); we add only the enforcement layer. **(1) Provider foundation** — provider-neutral registry + port + `PolicyEnforcedSandboxBackend` + fake adapter + conformance suite; **(2) egress/secrets** — deny-all egress compilation, short-lived audience-bound secret refs, non-overridable snapshot excludes; **(3) lifecycle/patch-apply** — snapshot-in / patch-out over AC4, AC5-reviewed apply, cancellation, durable reaper/teardown. ~3–4 PRs.
- **AC8 — agentic browser, in scope, built-in-first (Wave 3):** the engine is the LangChain Playwright toolkit / browser-MCP surface behind `langchain-mcp-adapters`; we add only origin/consent/artifact enforcement. **(1) Supervised-worker + read-only foundation** — protocol + supervised Playwright child + local `DesktopBrowserMcpProvider` + exact-origin policy proxy + read/navigate/snapshot; **(2) downloads/uploads/side-effects** — staging + AC4 offload for screenshots/downloads, AC5 upload refs, per-action approval for submit/upload/download; **(3) profiles/consent** — persistent profiles, interactive takeover, consent sheet, cross-workspace isolation, orphan cleanup. ~4–5 PRs.
