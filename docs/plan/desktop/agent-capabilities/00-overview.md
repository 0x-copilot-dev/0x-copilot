...
**Precedence:** [AC1](01-ac1-desktop-capability-foundation.md) is the single normative source for the frozen v1 contracts — the on-disk layout and root (§8), the canonical storage envelope and `ArtifactRefV1` (§7.4–§7.5), grant modes and capability vocabulary (§7.3), the broker path/headers/handshake (§6.3), and the runtime-store/event-bus selectors (§6.2). Where this overview and AC1 (or the conforming AC2/AC4/AC5) differ on any of those, **AC1 wins** and this document is treated as stale until regenerated. Overview §8/§10/§12/§13.1/§14 are summaries that defer to AC1; do not build from their prose.

> **LIGHT-store exception (storage envelope + session layout only).** The §25 decision closed in favor of a single-writer LIGHT store ([AC2](02-ac2-file-session-store.md)). AC2-light drops AC1 §7.4's `global_sequence_no`/`previous_stream_hash` and AC1 §8's `CURRENT`/`generations/`/`pending/`/`locks/`/`quarantine/` layout, and moves the content-addressed object store into AC2. **For the record envelope and session layout only, AC2-light is the built design and AC1 §7.4/§8 must be revised to match in a follow-up AC1 minor.** Every other AC1 frozen contract (grant modes §7.3, `ArtifactRefV1` §7.5, broker §6.3, activation predicate §6.2) is unchanged and remains normative.
...
> **Normative source: [AC1 §8](01-ac1-desktop-capability-foundation.md) and [AC2 §Filesystem layout](02-ac2-file-session-store.md).** The tree below is an orienting summary. With the LIGHT store decision (§25 closed), the shipping layout is a **flat `events.jsonl` per conversation** — **no** `CURRENT`, `generations/`, `pending/`, `locks/`, or `quarantine/` tree, because a single writer needs no copy-on-write or cross-process lock machinery. The root is `agent-data/v1` (not `agent-runtime/v1`), and grant material is **encrypted and stored outside `agent-data`**, under `capability-broker/v1/`. **Note:** AC1 §8 still depicts the heavy `CURRENT`/`generations/` layout and must be revised to this light layout in a follow-up AC1 minor; where they differ, **AC2-light is the built design**. If this summary and AC2 disagree, AC2 wins.
...
> **Normative source: [AC2 §Typed contracts / §Append and durability protocol](02-ac2-file-session-store.md).** This section is a conceptual summary. With the LIGHT store, the canonical envelope is the flat `FileSessionRecordV1`; there is **no** session-global counter and **no** `storage.batch_committed` marker — a single in-process writer appends one line per record, serialized by a per-conversation `asyncio.Lock`. Ordering uses the existing per-run `sequence_no`. **Note:** AC1 §7.4's frozen `FileStoreRecordV1` still lists `global_sequence_no` and `previous_stream_hash`; those are heavy-variant fields the light store drops and AC1 §7.4 must be revised to match. Build from AC2-light, not from AC1 §7.4 or the illustrative shape below.
