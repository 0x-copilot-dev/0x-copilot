...
- **Built-in-first (do not reinvent the framework):** Use the DeepAgents/LangChain built-in as the engine — DeepAgents interpreters (Monty), `SandboxBackendProtocol` (remote sandbox), LangChain Playwright toolkit / browser-MCP, `langchain-mcp-adapters`, `HumanInTheLoopMiddleware`, LangGraph savers — and add only the thin enforcement layer (approval/budget/event/persistence) we require. Do not reinvent what the framework provides. This is not aspirational: the delivered file store already reuses the LangGraph `AsyncSqliteSaver` as its graph/approval checkpointer ([`deep_agent_builder.py`](../../../../services/ai-backend/src/agent_runtime/execution/deep_agent_builder.py)), the desktop approval path rides the DeepAgents-native `interrupt_on` interrupt rather than a bespoke pause engine ([`factory.py`](../../../../services/ai-backend/src/agent_runtime/execution/factory.py)), and `/workspace/`, `/drafts/`, `/subagents/`, and `/large_tool_results/` are DeepAgents `CompositeBackend` routes, not a hand-rolled virtual filesystem. AC6/AC7/AC8 extend that same discipline to interpreters, sandboxes, and browser control.
...
**Wave 3 is in scope, not deferred.** AC6 (Monty code mode), AC7 (remote sandbox), and AC8 (agentic browser) are each **wanted and planned** — status **Planned — in scope** in the [PR index](README.md#pr-index) — not a speculative "maybe later" tier. Each is an **epic decomposed into multiple ordered sub-PRs** (see [PR decomposition](README.md#pr-decomposition-each-ac-is-an-epic)): AC6 = interpreter foundation → policy-engine prereq → parity; AC7 = provider foundation → egress/secrets → lifecycle/patch-apply; AC8 = supervised-worker + read-only foundation → downloads/uploads/side-effects → profiles/consent. Per the **built-in-first** principle (§4), each builds on the DeepAgents/LangChain built-in — the Monty `InterpreterPort`, `SandboxBackendProtocol`, and the LangChain Playwright toolkit / browser-MCP behind `langchain-mcp-adapters` — and adds only the thin approval/budget/event/persistence enforcement layer.
