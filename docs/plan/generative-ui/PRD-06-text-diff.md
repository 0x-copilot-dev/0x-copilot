...
**Goal:** the VSCode/Cursor-style red/green inline diff for _text-shaped_ surfaces. A dependency-free word-level differ in chat-surface, consumed by the email tier-1 renderer and the message/doc archetypes, with hunk structure that PRD-09 will use for per-hunk accept.
