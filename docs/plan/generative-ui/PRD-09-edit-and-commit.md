...
**Goal:** the *edit* in accept/decline/edit, end-to-end on the flagship journey (email draft → review → **edit** → approve → send), generalized so record-field edits work the same way. Reviewer edits flow INTO the committed payload; commit is idempotent, precondition-checked, audited, fail-closed.
