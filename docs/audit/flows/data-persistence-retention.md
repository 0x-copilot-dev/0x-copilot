...
> **Correction (2026-08-06) — the C7 field-encryption control no longer exists in the tree.**
> This document cites `agent_runtime/persistence/encryption.py` (envelope AES-256-GCM per
> column, AAD bound to `(table, column, org_id)`) as an implemented control, in the store
> inventory and again under "Field-encryption envelope". **That module has been deleted**,
> along with its `_aws_kms_client` and the `encrypt_existing_columns` backfill.
>
> It was never a control on the shipping product. Field encryption applied **only** to the
> Postgres adapter's column codec, which `e03840ed` removed with the rest of the Postgres
> storage backend; the desktop file store is plaintext-at-rest by design with
> `NullFieldEncryption` as the default (cluster 03 §F11, a documented and locked
> "plaintext-first" decision). After the adapter went, nothing in `src` referenced
> `FieldEncryption` at all and `RUNTIME_FIELD_ENCRYPTION` had no consumer and no settings
> knob, so the code was removed as an orphan rather than kept as evidence.
>
> **For a compliance reviewer:** treat encryption-at-rest for ai-backend data as **not
> implemented in the product**, not merely unevidenced. The OS-keychain-wrapped at-rest
> option cluster 03 suggested as a desktop follow-up remains the open path. The rows below
> are left as written so the change is visible rather than silently rewritten.
...
- **Deletion under legal hold:** postgres path 409s (`postgres/runtime_api_store.py:2116-2122`) with an acknowledged TOCTOU race (`:2087-2090`); file path skips held sessions. But no code path ever _creates_ a hold (no INSERT into `runtime_legal_holds`, no setter for the metadata flag).
