...
| ----- | ------------------------------------------------------------ | ------------------------ | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 6     | **Projects** (multi-thread workspace)                        | ✅ SHIPPED (+ Phase 6.5) | `destinations/projects-prd.md` + `projects-extensions-prd.md`   | —                                                                                                                                                                                     |
| 7     | **Library** (files + pages + datasets + retrieval)           | ✅ SHIPPED (+ Phase 7.5) | `destinations/library-prd.md`                                   | Q3 (library page output target wire shape)                                                                                                                                            |
| 7.5   | **Library retrieval** (BM25 + pgvector + RRF + embeddings)   | ✅ SHIPPED               | (rolled into library-prd §6)                                    | —                                                                                                                                                                                     |
| 8     | **Agents** (skill cards, agent identity, registry)           | ✅ SHIPPED               | `destinations/agents-prd.md`                                    | Q11 (agent_version_pin selection UI — landed)                                                                                                                                         |
| 9     | **Home redesign** (triage-first morning briefing + dev seed) | ✅ SHIPPED               | `destinations/home-prd.md` (supersedes Phase 2 7-section model) | —                                                                                                                                                                                     |
| 10    | **Tools** (full destination)                                 | ✅ SHIPPED               | `destinations/tools-prd.md`                                     | Q1 (code-routines executor + sandbox — landed via P10-A3, in-process v1)                                                                                                              |
| 11    | **Connectors** (full destination)                            | ✅ SHIPPED               | `destinations/connectors-prd.md`                                | Q6 (HMAC-of-payload signature UI — landed via P11-A3 webhook manager + signer)                                                                                                        |
| 12    | **Team + Memory + ⌘K palette + polish**                      | ✅ SHIPPED               | `destinations/team-memory-cmdk-prd.md`                          | Q9/Q10 (proposal extractor + ⌘K palette — landed via P12-A4+A5 + P12-B3), Q14 (Settings UI — landed via P12-B4), Q12 (admin force-reassign — STAYS DEFERRED, controlled handoff only) |

**Phase 9 (Home redesign) — why it pre-empted the original "Phase 9 = Tools" slot.** Phase 2 shipped Home's route + 6 stub composers (`pinned_chats`, `recent_runs`, `favorite_tools`, `todays_focus`, `upcoming_meetings`, `agent_activity`). In dev with no seed data, Home renders six empty cards — unsellable. Phase 9 (a) reframes Home around the "Does anything need me? / What's new? / What's today? / Where do I pick up?" four-question model, dropping pinned_chats / favorite_tools / starred_projects as standalone sections, merging today widgets into one timeline, and demoting the activity feed to a right-rail subordinate; and (b) adds a `dev_seed.yaml` + `DevSeedRunner` (mirrors `dev_personas.yaml` + `PersonaLoader`) so dev personas land with realistic density. Also fixes the `/v1/home/stream` 404 (facade was only proxying `/v1/home`, not the SSE stream). See `destinations/home-prd.md` for the full rewrite.

**Routines §9.7 deferral mapping (binding):**

| Routines §9.7 Q                                       | Wave label in §9.7                           | Lands in impl-plan phase                                                        |
| ----------------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------- |
| Q1 Code-routines executor                             | "Wave 6"                                     | **Phase 10 Tools** (executor + sandbox is tool infrastructure)                  |
| Q3 Library page output mode                           | "Phase 5" (wire shape) + Phase 7 (UI polish) | Phase 5 ships wire; Phase 7 polishes editor                                     |
| Q6 HMAC signature webhook                             | "Wave 5+"                                    | Phase 11 Connectors (consolidated webhook UX)                                   |
| Q9 Atlas-proposed cron suggestions                    | "Phase 5/6"                                  | Phase 12 (⌘K palette suggestion engine; tied to memory + agent context)         |
| Q10 Auto-extracted "Make this a routine?" CTA         | "Phase 6"                                    | Phase 12 (post-run CTA on chats canvas → command palette flow)                  |
| Q12 Admin force-reassign owner                        | "out of scope"                               | Re-evaluate at Phase 12 (Team destination + admin workflows)                    |
| Q13 Routine forking / templates                       | "Wave 5+"                                    | Phase 8 Agents (templating is an agent-registry concern; Routines just consume) |
| Q14 Settings UI for tenant/user notification defaults | "Wave 6"                                     | Phase 12 (Settings + Team destination)                                          |
