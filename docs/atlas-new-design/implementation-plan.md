...
### Phase 0.6 — Token-usage tracking (PREREQUISITE; runs in parallel with Phase 0.5)

Every LLM call across the monorepo flows through a single tracker that records `(source_kind, source_id, model, tokens, cost)` to a new `llm_token_usage` table. Mandatory consumer for Phases 1, 3, 5, 6, 11. See cross-audit §5.5.

| Agent                | Branch                       | Exclusive files                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Prereqs                    | Deliverables                                                                                                                                                | Test gates                                                                                                                                           |
| -------------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TU-1 Token Usage** | `worktree-agent-token-usage` | `packages/api-types/src/usage.ts` (NEW), `services/backend/src/backend_app/usage/` (NEW: routes.py, service.py, store.py, schema.sql), `services/backend-facade/src/backend_facade/usage_routes.py` (NEW), `services/ai-backend/src/agent_runtime/observability/llm_call_tracker.py` (NEW), `services/ai-backend/src/agent_runtime/execution/deep_agent_builder.py` (single integration point — wrap every provider invocation), CI guard (import-graph check forbidding provider SDK imports outside the tracker), Postgres migration, all related tests | none (independent of SP-1) | `llm_token_usage` table + tracker + `/v1/usage` endpoints (rollup + drill-down) + CI guard + retrofit of existing run pipeline so every chat run lands rows | `backend` + `backend-facade` + `ai-backend` full suites; cross-tenant isolation; rows-emitted-per-run assertion; cost-computation pricing-table test |
