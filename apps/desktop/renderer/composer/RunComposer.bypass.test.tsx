...
/**
 * The Electron capability bridge, answering with no grants.
 *
 * Required, not incidental: `AssistantComposer` gates the bypass slot on the
 * folder-grant CAPABILITY, because bypass only ever applies inside a folder the
 * user granted with write permission — so a host with no grant port has nothing
 * bypass could permit and must show no pill. `bridgeWorkspaceGrantPort()` reads
 * `window.bridge`, so without this the composer under test is a web-shaped one
 * and the pill is correctly absent.
 *
 * It answers with an EMPTY grant list on purpose. The pill's availability is a
 * question about the capability, not about how many folders are attached; the
 * runtime re-checks the actual grant before any pause is skipped.
 */
function installGrantBridge(): void {
  (globalThis.window as unknown as { bridge?: WindowBridge }).bridge = {
    ipc: {
      invoke: (async () => []) as unknown as WindowBridge["ipc"]["invoke"],
      on: () => () => {},
    },
  };
}
...
describe("RunComposer — filesystem bypass pill", () => {
  it("shows no pill at all without the folder-grant capability", async () => {
    // The master switch says YES and the host still shows nothing, because the
    // gate is the capability, not the switch: with no grant port there is no
    // folder bypass could ever cover. Asserted here, on the real host, rather
    // than only in the package — this is the composition that actually ships,
    // and it is the one that would regress if the gate were dropped as a merge
    // convenience.
    const dispatch = vi.fn(
      async (_request: RunStartRequest): Promise<void> => {},
    );
    render(
      <TransportProvider transport={fakeTransport(true)}>
        <RunComposer
          dispatch={dispatch}
          disabled={false}
          placeholder="Send a message…"
    ...
    expect(
      screen.queryByRole("button", { name: /Execution mode:/ }),
    ).toBeNull();
  });
