...
export interface ArchivedSourcesState {
  ...
  setSources: (
    next: SourceEntryMap | ((current: SourceEntryMap) => SourceEntryMap),
  ) => void;
  loading: boolean;
  error: string | null;
  reseed: (entries: readonly SourceEntry[]) => void;
}
...
export function useArchivedSources(
  ...
  const reseed = useCallback((entries: readonly SourceEntry[]) => {
    setSources(seedSourceMap(entries));
    setError(null);
  }, []);
  ...
  return { sources, setSources, loading, error, reseed };
}
