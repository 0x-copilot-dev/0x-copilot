...
describe("applyRuntimeEvent", () => {
  it("concatenates model deltas and reconciles the final assistant response", () => {
    ...
    items = applyRuntimeEvent(items, event({
      event_id: "event_1",
  ...
  it("projects tool, subagent, and reasoning events into run activity", () => {
    ...
    items = applyRuntimeEvent(items, event({
      ...
      payload: { task_id: "task_123", subagent_name: "researcher", status: "started" }
    }));
    items = applyRuntimeEvent(items, event({
      event_id: "tool_1",
      ...
      span_id: "call_123",
      payload: { tool_name: "doc_search", call_id: "call_123", status: "running" }
    }));
  ...
  it("ignores heartbeat events and unsupported non-text envelopes", () => {
    ...
    expect(applyRuntimeEvent(items, event({
