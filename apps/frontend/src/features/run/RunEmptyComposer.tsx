...
// pill · attach · send) bound to the SAME web substrate touchpoints the FTUE
// composer uses (`useOnboardingComposerModels` — the live `/v1/agent/models`
// catalog — plus the reused ChatScreen attachment adapter / file picker / `+`
// menu). Mirrors the desktop `RunEmptyComposer`; the two hosts cannot share code
// (`apps/* → apps/*` is banned), so they duplicate the same wiring over the
...
// (FR-3.25). Submitting / error / readiness come down through `ctx`.
...
export function RunEmptyComposer({
  ...
  const handleSubmit = useCallback(
    ...
    [composerModels, selectedModel, onStartRun],
  );
