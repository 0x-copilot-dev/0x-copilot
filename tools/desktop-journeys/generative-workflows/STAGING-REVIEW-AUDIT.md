...
| GSQA-004 | Blocker  | Open      | Host test environment      | macOS has not granted the Codex-hosted `osascript` process Accessibility access to System Events. The real folder chooser stayed open until the driver timed out. After the user enabled Accessibility, the retry host still reported `UI elements enabled = false`, and a direct UI query failed with macOS error `-25211`. | Second real installed-payload G1 run; two native automation probes                                               | Grant Accessibility to the actual Codex application, then fully relaunch Codex so the TCC decision is inherited by new shell/`osascript` processes. Do not replace the native grant with test seeding or a facade write. Then rerun G1 and G2 unchanged. |
...
| 2026-07-27          | Retried the host preflight after the user enabled Accessibility.                     | System Events process enumeration worked, but UI automation remained denied: `UI elements enabled = false`; direct window access failed with macOS error `-25211`. No model call was made.     |
