#!/usr/bin/env python3
"""Journey J3 — SHORT window: no surface is unreachable, document still frozen.

Companion to `shell_document_overflow.py` (J1/J2), which sweeps every settings
section and rail destination at the default window size and checks the document
never gains scrollable overflow. This one adds the axis that one cannot see: it
resizes the REAL window short and checks the surfaces that only break there.

Two invariants hold together in this frame, and each is only safe because of the
other:

  A. The document must never be scrollable. `desktop.css` declares
     `body { overflow: hidden }`, mirroring the web host
     (`apps/frontend/src/styles.css`). J1/J2 own the root-cause half of this
     (nothing escapes to the initial containing block); this journey re-checks the
     outcome half at short heights, where a latent escape becomes real overflow.

  B. Because of A, every full-window surface must own its OWN internal scroll.
     Anything that sizes itself past the frame, or leans on the document to
     scroll, becomes permanently unreachable once A is enforced — a
     defense-in-depth fix that silently creates an unreachable-content bug. Two
     surfaces render before the shell and are full-window: `.loginx-shell`
     (sign-in) and `.fr` (FTUE). Both regressed in exactly this way:
     `.loginx-shell` was `min-height: 100vh` with no internal overflow, and `.fr`
     inherited `min-height: 100vh` from the shared onboarding stylesheet, leaving
     33px of it — its footer included — clipped past the frame at EVERY height.

At the 1200x800 default both surfaces fit and every check here passes trivially,
so the whole walk runs at 1200x600 and 1200x420 and fails if the window manager
refused the size. Needs no provider key and starts no run: pure layout truth.

    python3 tools/desktop-journeys/shell-overflow/short_window_surfaces.py
...
def main() -> int:
    with DriverSession(name="short-window-surfaces") as s:
